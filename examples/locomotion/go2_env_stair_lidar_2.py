"""
go2_env_stair5_info.py
========================
Go2 stair-climbing RL environment with 4-value STAIR INFO observations.

Design — Minimal 4-value stair information for sim-to-real:
----------------------------------------------------------
Instead of a 5×3 terrain grid (which requires reliable LIDAR coverage),
this env provides the actor with exactly 4 values:

  1. edge_distance : Distance to next height change along forward axis (0–0.27m)
                     From LIDAR edge detection. 0.27m = "no edge in range"
  2. stair_height  : Step riser height in meters (always positive, user-provided)
  3. stair_depth   : Step tread depth in meters (always positive, user-provided)
  4. direction     : +1 ascending, -1 descending, 0 no stairs detected
                     Auto-detected from LIDAR height change sign

Sim-to-Real Pipeline:
  - In sim: edge_distance & direction computed from heightfield
  - On real robot: edge_distance & direction from L1 LIDAR edge detector
  - stair_height & stair_depth: user measures and provides as constants

The policy learns to combine:
  - Stair info (4 values): "what's ahead" — when to change gait
  - Proprioception (49 dims): "how am I doing" — gravity, joints, velocities
  - Gravity vector: "am I on stairs right now" — tilt from level

Training Robustness (noise on edge_distance & direction only):
  - ±20% noise on edge_distance (LIDAR measurement uncertainty)
  - 3% direction flip probability
  - 5% full blackout (edge→0.27, dir→0, forces proprioceptive fallback)
  - 10% latency (stale data)
  - ±5% noise on height/depth (minor user measurement error)
"""
...
# ======================================================================
# Terrain heightfield generation (with per-row variable depth)
# ======================================================================
...
def build_stair_terrain(terrain_cfg: dict) -> tuple:
    ...
    # --- Per-row step depth (variable, NOT curriculum-linked) ---
    step_depth_range = terrain_cfg.get("step_depth_range", [0.25, 0.25])
    if isinstance(step_depth_range, (int, float)):
        step_depth_range = [float(step_depth_range), float(step_depth_range)]

    step_depths_m = np.linspace(step_depth_range[0], step_depth_range[1], num_rows)
    np.random.shuffle(step_depths_m)  # decorrelate from height curriculum

    step_depths_cells = np.maximum(1, np.round(step_depths_m / h_scale).astype(int))
    step_depth_max_cells = int(np.max(step_depths_cells))
    ...
    stair_section_cells_max = num_steps * step_depth_max_cells

    flight_cells = stair_section_cells_max + flat_top_cells + stair_section_cells_max + flat_gap_cells
    ...
    for row_idx in range(num_rows):
        ...
        row_dc = int(step_depths_cells[row_idx])
        ...
        for _flight in range(num_flights):
            ...
            for s in range(num_steps):
                height = (s + 1) * sh
                hf[x:x + row_dc, y_start:y_end] = height
                x += row_dc
            # Pad remaining space in max section at top height
            top_height = num_steps * sh
            ascent_end = ascent_start + stair_section_cells_max
            if x < ascent_end:
                hf[x:ascent_end, y_start:y_end] = top_height
            x = ascent_end
            ...
            for s in range(num_steps):
                ...
                hf[x:x + row_dc, y_start:y_end] = height
                x += row_dc
            # Pad remaining at ground level
            descent_end = descent_start + stair_section_cells_max
            if x < descent_end:
                hf[x:descent_end, y_start:y_end] = 0
            x = descent_end
    ...
    terrain_info = {
        ...
        "step_depths_m": step_depths_m.tolist(),
        "total_x_m": total_x_m,
        "total_y_m": total_y_m,
    ...
    print(f"  Flights/row   : {num_flights}")
    print(f"  Steps/flight  : {num_steps}")
    print(f"  Step heights  : {[f'{h:.3f}m' for h in step_heights_m]}")
    print(f"  Step depths   : {[f'{d:.3f}m' for d in step_depths_m]}")
    print(f"  Terrain origin: {terrain_origin}")
    for i, (cx, cy, cz) in enumerate(adjusted_centers):
        print(f"  Row {i:2d} (h={step_heights_m[i]*100:.1f}cm, d={step_depths_m[i]*100:.0f}cm): "
              f"spawn=({cx:.2f}, {cy:.2f}, {cz:.2f})")
    print()
...
# ======================================================================
# Stair Info Configuration (replaces LidarScanConfig)
# ======================================================================

class StairInfoConfig:
    """
    Configuration for the 4-value stair information observation.

    Values:
      [0] edge_distance : 0..max_range (meters), scaled by edge_dist_scale
      [1] stair_height  : always positive (meters), scaled by height_scale
      [2] stair_depth   : always positive (meters), scaled by depth_scale
      [3] direction     : +1 ascending, -1 descending, 0 flat

    In simulation: edge_distance and direction are computed from heightfield.
    On real robot: edge_distance and direction from LIDAR edge detector,
                   height and depth from user measurement.
    """
    NUM_STAIR_INFO = 4
    ...
    def __init__(self, cfg: dict, device):
        ...
        self.edge_max_range = float(edge_cfg.get("max_range", 0.27))
        self.edge_threshold = float(edge_cfg.get("threshold", 0.02))
        self.edge_num_samples = int(edge_cfg.get("num_samples", 14))

        # Observation scaling (to bring values into ~[0, 1] range)
        self.edge_dist_scale = float(cfg.get("edge_dist_scale", 1.0 / 0.27))
        self.height_scale = float(cfg.get("height_scale", 1.0 / 0.20))
        self.depth_scale = float(cfg.get("depth_scale", 1.0 / 0.30))
        self.direction_scale = float(cfg.get("direction_scale", 1.0))

        # Noise model (scales with DR level)
        noise_cfg = cfg.get("noise", {})
        self.edge_noise_std = float(noise_cfg.get("edge_noise_std", 0.04))
        self.height_noise_std = float(noise_cfg.get("height_noise_std", 0.01))
        self.depth_noise_std = float(noise_cfg.get("depth_noise_std", 0.01))
        self.direction_flip_prob = float(noise_cfg.get("direction_flip_prob", 0.03))
        self.full_blackout_prob = float(noise_cfg.get("full_blackout_prob", 0.05))
        self.latency_prob = float(noise_cfg.get("latency_prob", 0.10))

        # Pre-compute edge sample points along forward axis
        self.edge_sample_x = torch.linspace(
            0.0, self.edge_max_range, self.edge_num_samples, device=device)

        print(f"\n[StairInfo] Configuration:")
        print(f"  Edge detection  : range=0..{self.edge_max_range}m, "
              f"threshold={self.edge_threshold}m, samples={self.edge_num_samples}")
        print(f"  Scaling         : edge={self.edge_dist_scale:.1f}, "
              f"height={self.height_scale:.1f}, depth={self.depth_scale:.1f}")
        print(f"  Noise:")
        print(f"    edge_std={self.edge_noise_std}m, height_std={self.height_noise_std}m, "
              f"depth_std={self.depth_noise_std}m")
        print(f"    dir_flip={self.direction_flip_prob}, blackout={self.full_blackout_prob}, "
              f"latency={self.latency_prob}")
        print()


# ======================================================================
# Go2Env with Stair Info
# ======================================================================
...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        if torque_lim_cfg is not None:
            ...
            self._torque_limits = torch.full(
                (self.num_pos_actions,), 23.7, device=gs.device, dtype=gs.tc_float)

        # ----------------------------------------------------------
        # Stair info setup (replaces LIDAR)
        # ----------------------------------------------------------
        stair_info_cfg = env_cfg.get("stair_info", {})
        self._stair_cfg = StairInfoConfig(stair_info_cfg, gs.device)
        ...
        if self._use_terrain:
            ...
            self._terrain_step_depths = self._terrain_info["step_depths_m"]
            ...
            self._hf_heights = torch.tensor(
                self._hf, device=gs.device, dtype=gs.tc_float) * self._v_scale

            # Per-row step heights and depths as tensors
            self._terrain_step_heights_t = torch.tensor(
                self._terrain_step_heights, device=gs.device, dtype=gs.tc_float)
            self._terrain_step_depths_t = torch.tensor(
                self._terrain_step_depths, device=gs.device, dtype=gs.tc_float)
        else:
            self._num_terrain_rows = 1
            self._terrain_row_centers = [(0.0, 0.0, 0.0)]
            self._terrain_step_heights_t = torch.zeros(1, device=gs.device, dtype=gs.tc_float)
            self._terrain_step_depths_t = torch.full((1,), 0.25, device=gs.device, dtype=gs.tc_float)
        ...
        self.motors_dof_idx = [self.robot.get_joint(name).dof_start
        ...
        for name in self.reward_scales.keys():
            ...
            if not hasattr(self, fn_name):
                raise AttributeError(
                    f"Reward function '{fn_name}' not found. "
        ...
        # ----------------------------------------------------------
        # Body contact penalty tracking (thigh links should not touch ground)
        # ----------------------------------------------------------
        self._has_body_contact_penalty = "body_contact" in self.reward_scales and self.reward_scales["body_contact"] != 0.0
        self._body_contact_link_indices = []
        if self._has_body_contact_penalty:
            self._init_body_contact_tracking()
        ...
        # ----------------------------------------------------------
        # Stair info buffers (4 values per env)
        # ----------------------------------------------------------
        self._stair_info_buf = torch.zeros(
            (self.num_envs, StairInfoConfig.NUM_STAIR_INFO),
            device=gs.device, dtype=gs.tc_float)
        self._stair_info_prev = torch.zeros_like(self._stair_info_buf)
        self._stair_info_clean = torch.zeros_like(self._stair_info_buf)

        print(f"[Go2Env] Stair info buffers: ({self.num_envs}, {StairInfoConfig.NUM_STAIR_INFO})")
        ...
        # Stair info noise level (scales with DR level)
        self._stair_noise_level = 0.0
    ...
    # ==========================================================
    # Stair Info: edge detection + observation computation
    # ==========================================================

    def _compute_edge_detection(self):
        """
        Detect nearest height change along each robot's forward axis.

        Scans 14 points from 0.0 to 0.27m ahead in body frame.
        Returns:
          edge_distance : (num_envs,) distance to first edge, or max_range
          direction     : (num_envs,) +1 ascending, -1 descending, 0 flat
        """
        if not self._use_terrain or not self._stair_cfg.enabled:
            max_r = self._stair_cfg.edge_max_range if self._stair_cfg.enabled else 0.27
            return (
                torch.full((self.num_envs,), max_r, device=self.device, dtype=gs.tc_float),
                torch.zeros(self.num_envs, device=self.device, dtype=gs.tc_float),
            )

        sample_x = self._stair_cfg.edge_sample_x      # (num_samples,)
        num_s = self._stair_cfg.edge_num_samples

        # Extract yaw for body→world rotation
        qw = self.base_quat[:, 0]
        qx = self.base_quat[:, 1]
        qy = self.base_quat[:, 2]
        qz = self.base_quat[:, 3]
        ...
        # Transform sample points to world frame (y=0, center line only)
        lx = sample_x.unsqueeze(0)                     # (1, num_s)
        wx = self.base_pos[:, 0:1] + cos_y * lx        # (N, num_s)
        wy = self.base_pos[:, 1:2] + sin_y * lx        # (N, num_s)

        # Terrain heights at sample points (absolute)
        heights = self._get_terrain_height(wx, wy)      # (N, num_s)

        # Consecutive height differences
        dh = heights[:, 1:] - heights[:, :-1]           # (N, num_s-1)

        # Find first significant height change per env
        threshold = self._stair_cfg.edge_threshold
        significant = torch.abs(dh) > threshold          # (N, num_s-1)
        has_edge = significant.any(dim=1)                 # (N,)
        first_idx = significant.to(torch.int32).argmax(dim=1)  # (N,)

        # Edge distance = midpoint x between the two samples
        mid_x = (sample_x[:-1] + sample_x[1:]) / 2      # (num_s-1,)
        edge_dist = mid_x[first_idx]                      # (N,)
        edge_dist = torch.where(has_edge, edge_dist,
                                torch.full_like(edge_dist, self._stair_cfg.edge_max_range))

        # Direction from height change sign
        batch_idx = torch.arange(self.num_envs, device=self.device)
        edge_dh = dh[batch_idx, first_idx]
        direction = torch.where(has_edge, torch.sign(edge_dh),
                                torch.zeros_like(edge_dh))

        return edge_dist, direction

    def _update_stair_info(self):
        """
        Compute 4-value stair info for actor observations (with noise).

        Values:
          [0] edge_distance * scale  — from heightfield edge detection + noise
          [1] stair_height  * scale  — from terrain row config + noise
          [2] stair_depth   * scale  — from terrain row config + noise
          [3] direction     * scale  — from edge detection + flip noise
        """
        if not self._stair_cfg.enabled:
            return

        # Save previous for latency
        self._stair_info_prev[:] = self._stair_info_buf[:]

        # --- Compute raw edge detection ---
        edge_dist, direction = self._compute_edge_detection()

        # --- Per-env stair height and depth (from terrain row assignment) ---
        stair_height = self._terrain_step_heights_t[self._env_terrain_row]  # (N,)
        stair_depth = self._terrain_step_depths_t[self._env_terrain_row]    # (N,)

        # --- Compute clean version for critic (before noise) ---
        self._stair_info_clean[:, 0] = edge_dist * self._stair_cfg.edge_dist_scale
        self._stair_info_clean[:, 1] = stair_height * self._stair_cfg.height_scale
        self._stair_info_clean[:, 2] = stair_depth * self._stair_cfg.depth_scale
        self._stair_info_clean[:, 3] = direction * self._stair_cfg.direction_scale

        # --- Apply noise (scales with DR level) ---
        noise_lvl = self._stair_noise_level

        # 1. Edge distance noise (±20% at full noise)
        if self._stair_cfg.edge_noise_std > 0 and noise_lvl > 0:
            edge_dist = edge_dist + torch.randn_like(edge_dist) * self._stair_cfg.edge_noise_std * noise_lvl
            edge_dist = edge_dist.clamp(0.0, self._stair_cfg.edge_max_range)

        # 2. Height measurement noise (±5%)
        if self._stair_cfg.height_noise_std > 0 and noise_lvl > 0:
            stair_height = stair_height + torch.randn_like(stair_height) * self._stair_cfg.height_noise_std * noise_lvl

        # 3. Depth measurement noise (±5%)
        if self._stair_cfg.depth_noise_std > 0 and noise_lvl > 0:
            stair_depth = stair_depth + torch.randn_like(stair_depth) * self._stair_cfg.depth_noise_std * noise_lvl

        # 4. Direction flip (wrong sign from noisy LIDAR)
        if self._stair_cfg.direction_flip_prob > 0 and noise_lvl > 0:
            flip_mask = torch.rand(self.num_envs, device=self.device) < (self._stair_cfg.direction_flip_prob * noise_lvl)
            direction = torch.where(flip_mask, -direction, direction)

        # 5. Latency: some envs get previous step's stair info
        if self._stair_cfg.latency_prob > 0 and noise_lvl > 0:
            latency_mask = torch.rand(self.num_envs, device=self.device) < (self._stair_cfg.latency_prob * noise_lvl)
            # Only replace edge_distance and direction (LIDAR-sourced)
            edge_dist = torch.where(latency_mask, self._stair_info_prev[:, 0] / self._stair_cfg.edge_dist_scale, edge_dist)
            direction = torch.where(latency_mask, self._stair_info_prev[:, 3] / self._stair_cfg.direction_scale, direction)

        # 6. Full LIDAR blackout (edge and direction go dead, height/depth stay)
        if self._stair_cfg.full_blackout_prob > 0 and noise_lvl > 0:
            blackout = torch.rand(self.num_envs, device=self.device) < (self._stair_cfg.full_blackout_prob * noise_lvl)
            edge_dist = torch.where(blackout, torch.full_like(edge_dist, self._stair_cfg.edge_max_range), edge_dist)
            direction = torch.where(blackout, torch.zeros_like(direction), direction)

        # --- Scale and store ---
        self._stair_info_buf[:, 0] = edge_dist * self._stair_cfg.edge_dist_scale
        self._stair_info_buf[:, 1] = stair_height * self._stair_cfg.height_scale
        self._stair_info_buf[:, 2] = stair_depth * self._stair_cfg.depth_scale
        self._stair_info_buf[:, 3] = direction * self._stair_cfg.direction_scale
    ...
    def _assign_terrain_rows(self, envs_idx, curriculum_level: float):
        ...
        if max_row >= 2:
            ...
            rows[n_frontier:n_frontier + n_near] = gs_rand_int(near_min, near_max, (n_near,), gs.device)
        else:
            rows[n_frontier:n_frontier + n_near] = max_row

        n_easy = n - n_frontier - n_near
        easy_max = max(0, max_row - 3) if max_row >= 3 else 0
    ...
    # ==========================================================
    # Body contact tracking (thigh links — should NOT touch ground)
    # ==========================================================

    def _init_body_contact_tracking(self):
        """Find thigh link indices for body contact penalty.

        On the Go2, the thigh links are the main offenders when
        the robot crawls — they plant on stair treads for stability.
        Penalizing contact forces on these links eliminates crawling.
        """
        body_contact_names = self.env_cfg.get(
            "body_contact_names",
            ["FR_thigh", "FL_thigh"])
        ...
        for name in body_contact_names:
            if name in link_name_map:
                self._body_contact_link_indices.append(link_name_map[name])
            else:
                print(f"[WARN] Body contact link '{name}' not found, skipping")

        cf = self.robot.get_links_net_contact_force()
        n_links = cf.shape[1] if cf is not None and len(cf.shape) >= 2 else 0
        self._body_contact_link_indices = [
            idx for idx in self._body_contact_link_indices if idx < n_links]

        self._body_contact_threshold = float(
            self.env_cfg.get("body_contact_threshold", 1.0))

        if self._body_contact_link_indices:
            print(f"[Go2Env] Body contact penalty ENABLED on links: "
                  f"{body_contact_names} → indices {self._body_contact_link_indices}")
        else:
            print(f"[WARN] No valid body contact links found, penalty disabled")
            self._has_body_contact_penalty = False
    ...
    def _rebuild_obs_noise_vec(self, level: float):
        ...
        num_proprio = 3 + 3 + 3 + 12 + 12 + self.num_actions
        ...
        # Stair info noise handled in _update_stair_info(), leave zeros here
        if self._stair_cfg.enabled:
            nv[num_proprio:] = 0.0
    ...
    def _get_dr_level(self, terrain_level: float) -> float:
        ...
        if terrain_level < self._dr_terrain_gate:
            ...
            progress = (terrain_level - self._dr_terrain_gate) / max(1e-6, 1.0 - self._dr_terrain_gate)
            return _lerp(self._dr_phase1_level, 1.0, _clamp01(progress))
    ...
    def _apply_curriculum_level(self, force: bool = False):
        ...
        # Stair info noise ramps with DR level
        self._stair_noise_level = dr_lvl
        ...
        self._delay_max_cur = int(round(_lerp(
            float(self.delay_easy_max), float(self._max_delay_steps), dr_lvl)))
        ...
        if self._cmd_curriculum:
            ...
            for key, full_range in self._cmd_full_ranges.items():
                ...
                self._cmd_cur_ranges[key] = [center - half_span * frac, center + half_span * frac]
        else:
            self._cmd_cur_ranges = {k: list(v) for k, v in self._cmd_full_ranges.items()}
        ...
        self.extras["curriculum"].update({
            ...
            "stair_noise_level": float(self._stair_noise_level),
        })
    ...
    def _randomize_kp_kd(self, envs_idx, t_sample: float):
        ...
        if self.pls_enable:
            if "kp_factor_range" in self.env_cfg:
                ...
                self._kp_factors[envs_idx] = gs_rand_float(kpf_range[0], kpf_range[1], (n, self.num_pos_actions), gs.device)
            if "kd_factor_range" in self.env_cfg:
                kdf_range = _lerp_range(self.kd_factor_easy, self.kd_factor_hard, t_sample)
                self._kd_factors[envs_idx] = gs_rand_float(kdf_range[0], kdf_range[1], (n, self.num_pos_actions), gs.device)
        else:
    ...
    def _randomize_gravity_offset(self, envs_idx, t_sample: float):
        ...
        self._gravity_offset[envs_idx] = gs_rand_float(go_range[0], go_range[1], (n, 3), gs.device)
    ...
    def _randomize_motor_strength(self, envs_idx, t_sample: float):
        ...
        self._motor_strength[envs_idx] = gs_rand_float(ms_range[0], ms_range[1], (n, self.num_pos_actions), gs.device)

    def _randomize_delay(self, envs_idx):
        n = len(envs_idx)
        max_d = max(self._min_delay_steps, min(self._delay_max_cur, self._max_delay_steps))
        self._delay_steps[envs_idx] = gs_rand_int(self._min_delay_steps, max_d, (n,), gs.device)
    ...
    def _apply_push(self):
        ...
        try:
            self.scene.sim.rigid_solver.apply_links_external_force(
                force=force, links_idx=[self._base_link_idx], envs_idx=self._all_envs_idx)
        except TypeError:
            self.scene.sim.rigid_solver.apply_links_external_force(
    ...
    def _resample_commands(self, envs_idx):
        ...
        if self._compound_commands:
            self.commands[envs_idx, 0] = gs_rand_float(self._cmd_cur_ranges["lin_vel_x"][0], self._cmd_cur_ranges["lin_vel_x"][1], (n,), gs.device)
            self.commands[envs_idx, 1] = gs_rand_float(self._cmd_cur_ranges["lin_vel_y"][0], self._cmd_cur_ranges["lin_vel_y"][1], (n,), gs.device)
            self.commands[envs_idx, 2] = gs_rand_float(self._cmd_cur_ranges["ang_vel"][0], self._cmd_cur_ranges["ang_vel"][1], (n,), gs.device)
        else:
            ...
            for i, (key, dim) in enumerate(zip(["lin_vel_x", "lin_vel_y", "ang_vel"], [0, 1, 2])):
                ...
                if mask.any():
                    self.commands[envs_idx[mask], dim] = gs_rand_float(
                        self._cmd_cur_ranges[key][0], self._cmd_cur_ranges[key][1], (mask.sum(),), gs.device)
    ...
    def step(self, actions):
        ...
        # ---- Build actor observation (49 proprio + 4 stair info = 53) ----
        obs_parts = [
            ...
            self._applied_actions,                                                 # 16
        ]

        if self._stair_cfg.enabled:
            obs_parts.append(self._stair_info_buf)                                 # 4
    ...
    def _build_privileged_obs(self):
        ...
        # Copy full actor obs (includes stair info)
        ...
        # Clean stair info (no noise, privileged)
        if self._stair_cfg.enabled and (idx + StairInfoConfig.NUM_STAIR_INFO) <= priv_dim:
            self.privileged_obs_buf[:, idx:idx + StairInfoConfig.NUM_STAIR_INFO] = self._stair_info_clean
            idx += StairInfoConfig.NUM_STAIR_INFO
    ...
    def reset_idx(self, envs_idx):
        ...
        if "init_euler_range" in self.env_cfg:
            ...
            n = len(envs_idx)
            roll = gs_rand_float(math.radians(low_deg), math.radians(high_deg), (n,), gs.device)
            pitch = gs_rand_float(math.radians(low_deg), math.radians(high_deg), (n,), gs.device)
        ...
        # Reset stair info buffers
        if self._stair_cfg.enabled:
            self._stair_info_buf[envs_idx] = 0.0
            self._stair_info_prev[envs_idx] = 0.0
            self._stair_info_clean[envs_idx] = 0.0
    ...
    def _reward_base_height(self):
        if self._use_terrain:
            terrain_z = self._get_terrain_height(self.base_pos[:, 0], self.base_pos[:, 1])
    ...
    def _reward_body_contact(self):
        """Penalize contact forces on non-foot body links (thighs).

        This directly punishes the crawling strategy where the robot
        plants its knees/thighs on stair treads for stability.
        Returns sum of contact force magnitudes on penalized links.
        """
        if not self._has_body_contact_penalty or not self._body_contact_link_indices:
            return torch.zeros(self.num_envs, device=self.device, dtype=gs.tc_float)

        cf = self.robot.get_links_net_contact_force()
        penalty = torch.zeros(self.num_envs, device=self.device, dtype=gs.tc_float)
        for link_idx in self._body_contact_link_indices:
            force_mag = torch.norm(cf[:, link_idx, :], dim=1)
            penalty += (force_mag > self._body_contact_threshold).float()
        return penalty