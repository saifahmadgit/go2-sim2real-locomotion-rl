...
# =============================================================
# Default terrain configuration — single source of truth.
# Import this in train and eval scripts.
# =============================================================
DEFAULT_TERRAIN_CFG = {
    "enabled": True,
    "up_only": True,
...
def build_stair_terrain(terrain_cfg: dict) -> tuple:
    ...
    step_height_max = terrain_cfg.get("step_height_max", 0.20)

    # =================== NEW: up_only mode flag ===================
    up_only = terrain_cfg.get("up_only", False)
    # ==============================================================
    ...
    # =================== CHANGED: layout depends on mode ===================
    if up_only:
        # Up-only: stairs_up + platform (no down section, no gap)
        flight_cells = stair_section_cells + flat_top_cells
        total_x_cells = flat_before_cells + num_flights * flight_cells + flat_after_cells
    else:
        # Original: stairs_up + platform + stairs_down + gap
    ...
    for row_idx in range(num_rows):
        ...
        # Flat approach (height = 0)
        x += flat_before_cells

        if up_only:
            # =================== NEW: up-only stair layout ===================
            # Height accumulates monotonically across flights
            cumulative_height = 0

            for _flight in range(num_flights):
                # Stairs UP from current height
                for s in range(num_steps):
                    cumulative_height += sh
                    hf[x:x + step_depth_cells, y_start:y_end] = cumulative_height
                    x += step_depth_cells

                # Flat platform at current accumulated height
                hf[x:x + flat_top_cells, y_start:y_end] = cumulative_height
                x += flat_top_cells

            # Flat exit at final height (so robot doesn't fall off edge)
            hf[x:x + flat_after_cells, y_start:y_end] = cumulative_height
            # =================================================================
        else:
            # Original up-down layout (unchanged from v4)
            for _flight in range(num_flights):
                # Stairs UP
                for s in range(num_steps):
                    height = (s + 1) * sh
                ...
                # Flat gap between flights (height = 0)
                x += flat_gap_cells
    ...
    if up_only:
        stair_m_per_flight = stair_section_cells * h_scale
    else:
    ...
    mode_str = "UP-ONLY" if up_only else "UP-DOWN"
    print(f"\n[Terrain] Built {mode_str} stair terrain heightfield:")
    print(f"  Shape         : {hf.shape} cells  ({total_x_m:.1f}m × {total_y_m:.1f}m)")
    print(f"  Difficulty rows: {num_rows}")
    print(f"  Flights/row   : {num_flights} ({'ascending only' if up_only else 'up-down cycles'})")
    ...
    if up_only:
        max_elev = num_flights * num_steps * step_heights_m[-1]
        print(f"  Max elevation : {max_elev:.2f}m (hardest row, {num_flights} flights)")
...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        self._foot_reward_names = [
            "feet_air_time", "foot_slip", "foot_clearance",
            "foot_clearance_barrier",  # NEW (v5)
            "feet_stance",
        ]
        ...
        # ----------------------------------------------------------
        # DR level decoupling — two-phase training
        # None = DR tracks curriculum 1:1 (or fixed if no schedule)
        # ----------------------------------------------------------
        ...
        if dr_cfg is not None:
            ...
            print(f"[Go2Env] Two-phase DR ENABLED:")
            print(f"  Phase 1: DR={self._dr_phase1_level:.2f} "
                  f"(until terrain level >= {self._dr_terrain_gate:.2f})")
            print(f"  Phase 2: DR ramps {self._dr_phase1_level:.2f} → 1.0")
        else:
    ...
    def _init_foot_contact_tracking(self):
        ...
        if not foot_names:
            raise ValueError(
                "env_cfg['foot_names'] is required for foot contact rewards."
            )
        link_name_map = {}
        for i, link in enumerate(self.robot.links):
            name = getattr(link, "name", None)
    ...
    def _maybe_update_curriculum_on_reset(self, envs_idx):
        ...
        # =================== NEW (v5): also count forward_progress ===================
        if "forward_progress" in self.episode_sums:
            tracking_int = tracking_int + self.episode_sums["forward_progress"][envs_idx]
        # =============================================================================
    ...
    def _build_privileged_obs(self):
        ...
        if self._use_terrain and self._scan_n > 0 and (idx + self._scan_n) <= priv_dim:
            height_scan = self._compute_height_scan()
    ...
    def _reward_orientation_roll_only(self):
        """Penalize ROLL only, NOT pitch."""
        return torch.square(self.projected_gravity[:, 1])

    # =================== NEW (v5): pitch excess reward ===================
    def _reward_pitch_excess(self):
        """Penalize pitch outside the allowed range [min_deg, max_deg].

        Stair climbing requires forward pitch (nose tilts down as robot
        ascends). The allowed range is asymmetric:
          - Forward pitch up to ~40° is natural climbing tilt → no penalty
          - Backward pitch (< 0°) is always wrong on ascending stairs → penalise
          - Extreme forward pitch (> 40°) means face collision risk → penalise

        Uses base_euler[:, 1] which is pitch in degrees.
        Positive pitch = forward lean (nose down), which is correct for climbing.
        """
        pitch_deg = self.base_euler[:, 1]  # degrees, positive = forward lean
        min_pitch = float(self.reward_cfg.get("pitch_allowed_min_deg", 0.0))
        max_pitch = float(self.reward_cfg.get("pitch_allowed_max_deg", 40.0))

        # Penalty for pitching backward (below min)
        backward_excess = torch.clamp(min_pitch - pitch_deg, min=0.0)
        # Penalty for pitching too far forward (above max)
        forward_excess = torch.clamp(pitch_deg - max_pitch, min=0.0)

        # Sum both violations, squared
    ...
    # =================== NEW (v5): barrier-style foot clearance ===================
    def _reward_foot_clearance_barrier(self):
        """ONE-SIDED barrier penalty for swing foot height.

        Unlike foot_clearance which penalises deviation in BOTH directions
        (too high AND too low), this only penalises feet that are BELOW
        the target clearance. Feet lifted ABOVE the target get zero penalty.

        This encourages the "arc" foot trajectory needed for stairs:
        the policy is free to lift feet as high as it wants (good for
        clearing stair risers) but gets punished for not lifting enough.

        Uses a steeper (cubic) penalty for large shortfall to create
        a barrier-like gradient near the clearance threshold.
        """
        if not self._has_link_pos:
            return torch.zeros(self.num_envs, device=self.device, dtype=gs.tc_float)

        link_pos = self.robot.get_links_pos()
        target_height = self.reward_cfg.get("feet_height_target", 0.12)
        swing = (~self._foot_contact).float()
        penalty = torch.zeros(self.num_envs, device=self.device, dtype=gs.tc_float)
        ...
        for i, link_idx in enumerate(self._foot_link_indices):
            ...
            if self._use_terrain:
                terrain_z = self._get_terrain_height(foot_x, foot_y)
                foot_z_rel = foot_z - terrain_z
            else:
                foot_z_rel = foot_z

            # ONE-SIDED: only penalise when foot is BELOW target
            shortfall = torch.clamp(target_height - foot_z_rel, min=0.0)
            # Cubic barrier: steep gradient near threshold, explodes for large shortfall
            barrier_penalty = shortfall * shortfall * shortfall

            if self._has_link_vel:
                foot_vel_xy_norm = torch.norm(link_vel[:, link_idx, :2], dim=1)
                penalty += swing[:, i] * barrier_penalty * foot_vel_xy_norm
            else:
                penalty += swing[:, i] * barrier_penalty
