...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        # ===========================================================
        # TERRAIN — uses TerrainManager for both custom heightmap
        # and subterrain grid modes.
        #
        # Naming convention (matches eval script expectations):
        #   self.terrain = TerrainManager instance (height queries, spawn, boundary)
        #   self.ground  = Genesis entity (set_friction, physics)
        # ===========================================================
        from terrain_manager import TerrainManager

        terrain_cfg = env_cfg.get("terrain", {})
        self.terrain = TerrainManager(terrain_cfg, num_envs)

        if self.terrain.enabled:
            terrain_morph = self.terrain.build_terrain_morph()
            self.ground = self.scene.add_entity(terrain_morph)
        else:
        ...
        # Post-build: let TerrainManager cache the height field
        if self.terrain.enabled:
            self.terrain.post_build(self.ground)
        ...
        # ----------------------------------------------------------
        # Ground height buffer (terrain-aware rewards & debug)
        # ----------------------------------------------------------
        self._ground_height = torch.zeros(
    ...
    def _init_foot_contact_tracking(self):
        ...
        if not foot_names:
            raise ValueError("env_cfg['foot_names'] is required for foot contact rewards.")
    ...
    def _apply_curriculum_level(self, force: bool = False):
        ...
        self._delay_max_cur = int(round(_lerp(float(self.delay_easy_max), float(self._max_delay_steps), lvl)))
    ...
    def _randomize_friction(self, envs_idx, t_sample: float):
        ...
        # Use self.ground (the Genesis entity) for friction
    ...
    def _apply_push(self):
        ...
        if self._push_counter % self._push_interval == 0:
            ...
            dur_lo, dur_hi = self._push_duration_steps
    ...
    def step(self, actions):
        ...
        # Update ground height under each robot (terrain-aware rewards)
        if self.terrain.enabled:
            self._ground_height[:] = self.terrain.get_height_at_robot(self.base_pos[:, :2])
        ...
        # Boundary reset: terminate if robot wanders too far from spawn
        if self.terrain.enabled:
            self.reset_buf |= self.terrain.check_boundary(self.base_pos[:, :2])
    ...
    def _build_privileged_obs(self):
        ...
        idx = self.num_obs
        self.privileged_obs_buf[:, idx:idx+3] = self.base_lin_vel * self.obs_scales["lin_vel"]; idx += 3
        self.privileged_obs_buf[:, idx] = self._current_friction; idx += 1
    ...
    def reset_idx(self, envs_idx):
        ...
        # --- Terrain-aware spawn ---
        if self.terrain.enabled:
            spawn_pos = self.terrain.sample_spawn(
                envs_idx, base_init_z=float(self.base_init_pos[2])
            )
            ...
            self.base_pos[envs_idx] = self.base_init_pos
            # Random init z only on flat ground (terrain spawn already sets z)
    ...
    def _reward_base_height(self):
        # Terrain-aware: measure height above local ground, not absolute z
        height_above_ground = self.base_pos[:, 2] - self._ground_height
