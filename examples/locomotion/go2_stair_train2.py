...
def get_cfgs():
    ...
    # ================================================================
    # Terrain config — frontier-heavy with height scan
    # ================================================================
    terrain_cfg = {
        "enabled": True,
    ...
    curriculum_cfg = {
        ...
        # RELAXED thresholds (stairs are hard — old values were too strict)
        "ready_timeout_rate": 0.60,      # 60% survive to timeout (was 0.70)
        "ready_tracking": 0.45,          # Lower tracking bar (was 0.55, stairs slow robot)
        "ready_fall_rate": 0.35,         # Up to 35% falls OK (was 0.25)
        "ready_streak": 3,               # 3 consecutive checks (was 5)

        "hard_fall_rate": 0.50,          # Back off if 50%+ falling (was 0.40)
        "hard_streak": 2,

        "step_up": 0.02,                 # Faster advance (was 0.01)
        "step_down": 0.03,               # Fast retreat when struggling
        "cooldown_updates": 3,           # Reduced from 5
    ...
    reward_cfg = {
        ...
        "reward_scales": {
            ...
            "base_height": -0.5,            # Increased from -0.3 (now terrain-relative, can be stricter)
            "action_rate": -0.01,
...
def main():
    ...
    parser.add_argument("-e", "--exp_name", type=str, default="go2-stairs-v2")
    ...
    # ================================================================
    print("\n" + "=" * 70)
    print("  TRAINING CONFIG — STAIR CLIMBING v2 (terrain-relative rewards)")
    print("=" * 70)
    ...
    stair_tuned = {"foot_clearance", "lin_vel_z", "base_height", "similar_to_default", "forward_progress"}
    ...
    print(f"  lin_vel_z_deadzone : {reward_cfg['lin_vel_z_deadzone']}m/s ← NEW (allow z-vel on stairs)")
    print(f"  base_height/foot_clearance are now TERRAIN-RELATIVE (critical fix)")
    ...
    if cc.get("enabled"):
        ...
        print(f"    ready_streak         : {cc.get('ready_streak')} (relaxed)")
        print(f"    step_up              : {cc.get('step_up')} (faster advance)")
    print(f"  Spawn distribution     : 50% frontier / 30% near / 20% easy")
