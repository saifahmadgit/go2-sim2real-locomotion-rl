...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        if self.pls_enable:
            self.pls_kp_range = env_cfg["pls_kp_range"]       # [kp_min, kp_max]
            self.pls_kp_default = float(env_cfg["pls_kp_default"])
            self.pls_kp_action_scale = float(env_cfg["pls_kp_action_scale"])
            # Leg → joint mapping: each leg controls 3 consecutive joints
        ...
        # Set initial PD gains (will be overridden every step if PLS or manual PD)
        ...
        if self._has_leg_mass_dr:
            ...
            # Try to find hip links by common naming patterns
            hip_link_patterns = [
            ...
            if not found:
                # Fallback: use the child link of each hip joint
                for jname in hip_joint_names:
                    try:
                        joint = self.robot.get_joint(jname)
                        # Try child_link, then link attribute
            ...
            if len(self._hip_link_indices) == 4:
                ...
                print(f"[Go2Env] WARNING: Could not find all 4 hip links "
                      f"(found {len(self._hip_link_indices)}). Per-leg mass DR DISABLED.")
                print(f"  Available links: {sorted(_link_name_map.keys()) if _link_name_map else 'unknown'}")
                self._has_leg_mass_dr = False

        # ----------------------------------------------------------
        # Mid-episode dynamic payload
        # ----------------------------------------------------------
        self._dynamic_payload_enable = "dynamic_payload_range" in self.env_cfg
        if self._dynamic_payload_enable:
            self._dynamic_payload_interval = max(1, int(
                float(self.env_cfg.get("dynamic_payload_interval_s", 8.0)) / self.dt
            ))
            self._dynamic_payload_prob = float(self.env_cfg.get("dynamic_payload_prob", 0.3))
            self._dynamic_payload_counter = 0
            print(f"[Go2Env] Dynamic payload ENABLED: interval={self._dynamic_payload_interval} steps, "
                  f"prob={self._dynamic_payload_prob}")
        ...
        if self.push_force_range_hard is not None:
            ...
            # Push duration: convert seconds → steps
            ...
            # Stored force (held across steps during a push)
            self._push_stored_force = torch.zeros((self.num_envs, 3), device=gs.device, dtype=gs.tc_float)
            # Countdown: how many steps remain for current push
            self._push_remaining = torch.zeros((self.num_envs,), device=gs.device, dtype=torch.long)

            print(f"[Go2Env] Push duration: {self._push_duration_steps[0]}-{self._push_duration_steps[1]} steps "
                  f"({dur_cfg[0]*1000:.0f}-{dur_cfg[1]*1000:.0f} ms)")
        ...
        # ----------------------------------------------------------
        # Per-env DR buffers (for manual PD + privileged obs)
        # ----------------------------------------------------------
        # Multiplicative Kp/Kd DR factors (per-env, per-joint)
        ...
        # Per-leg (hip) mass shifts (4 legs)
        self._current_leg_mass_shifts = torch.zeros((self.num_envs, 4), device=gs.device, dtype=gs.tc_float)

        # Effective Kp/Kd used in manual PD (per-env, per-joint)
        # These are updated each step (PLS) or at reset (non-PLS)
        ...
        # Target DOF positions (stored for joint_tracking reward)
        ...
        # Dynamic payload curriculum
        self.dynamic_payload_easy = self.curr_cfg.get("dynamic_payload_easy", [-0.2, 0.5])
        self.dynamic_payload_hard = self.env_cfg.get("dynamic_payload_range",
                                                      self.env_cfg.get("mass_shift_range", self.dynamic_payload_easy))
        ...
        # ----------------------------------------------------------
        # Detect if manual PD torque is needed
        # Manual PD is used when:
        #   1. PLS is enabled (per-env stiffness from policy output)
        #   2. Per-joint Kp/Kd factor DR is enabled (per-env, per-joint variation)
        # This avoids reliance on Genesis supporting per-env set_dofs_kp.
        # ----------------------------------------------------------
        self._use_manual_pd = self.pls_enable or ("kp_factor_range" in self.env_cfg)
        if self._use_manual_pd:
            # Disable Genesis built-in PD by setting gains to 0
            ...
            print(f"[Go2Env] Manual PD torque mode ENABLED (PLS={self.pls_enable}, per-joint DR={('kp_factor_range' in self.env_cfg)})")
    ...
    def _init_foot_contact_tracking(self):
        ...
        if not link_name_map:
            raise RuntimeError(
                f"Could not enumerate robot link names. "
                f"Robot has {len(self.robot.links)} links but none have a 'name' attribute."
            )
        ...
        for fname in foot_names:
            if fname not in link_name_map:
                raise RuntimeError(
                    f"Foot link '{fname}' NOT FOUND in robot URDF.\n"
                    f"Available link names ({len(link_name_map)}): {sorted(link_name_map.keys())}"
                )
            self._foot_link_indices.append(link_name_map[fname])

        # Verify contact force API
        try:
        ...
        for fname, lidx in zip(foot_names, self._foot_link_indices):
            if lidx >= n_links_in_cf:
                raise RuntimeError(
                    f"Foot link '{fname}' idx {lidx} out of range (contact force has {n_links_in_cf} links)."
                )

        # Detect link position/velocity APIs for foot_slip/foot_clearance
        ...
        print(f"  contact_cf_shape : {cf_shape}")
        print(f"  threshold        : {self._foot_contact_threshold} N")
    ...
    def _rebuild_obs_noise_vec(self, level: float):
        """Build observation noise vector scaled by curriculum level."""
    ...
    def _apply_curriculum_level(self, force: bool = False):
        ...
        # Delay curriculum: max delay increases with level
    ...
    def _randomize_friction(self, envs_idx, t_sample: float):
        ...
        self._current_friction[envs_idx] = mu
    ...
    def _randomize_kp_kd(self, envs_idx, t_sample: float):
        """Randomise base Kp/Kd (for non-PLS mode) OR Kp/Kd factors (for PLS mode)."""
        self.extras.setdefault("domain_randomization", {})
        n = len(envs_idx)

        if self.pls_enable:
            # In PLS mode, policy controls Kp. We only randomise multiplicative factors.
            ...
            # Non-PLS: randomise absolute Kp/Kd + multiplicative factors
            if "kp_range" in self.env_cfg:
                ...
                # Apply multiplicative factors on top
                if "kp_factor_range" in self.env_cfg:
                    kpf_range = _lerp_range(self.kp_factor_easy, self.kp_factor_hard, t_sample)
                    self._kp_factors[envs_idx] = gs_rand_float(
                        kpf_range[0], kpf_range[1], (n, self.num_pos_actions), gs.device,
                ...
                # If not using manual PD, set globally (best-effort, last reset wins)
    ...
    def _randomize_mass(self, envs_idx, t_sample: float):
        """Mass/CoM DR with curriculum scaling."""
        self.extras.setdefault("domain_randomization", {})
        ...
        if "mass_shift_range" in self.env_cfg:
            ...
            self._current_mass_shift[envs_idx] = shift
        ...
        if "com_shift_range" in self.env_cfg:
            ...
            self._current_com_shift[envs_idx] = torch.tensor([dx, dy, dz], device=gs.device)

    def _randomize_gravity_offset(self, envs_idx, t_sample: float):
        """Gravity observation offset — covers IMU bias + slope perception error."""
    ...
    def _randomize_leg_mass(self, envs_idx, t_sample: float):
        """Per-leg (hip link) mass randomisation — creates asymmetric dynamics."""
        ...
        for i, link_idx in enumerate(self._hip_link_indices):
            ...
            try:
                ...
                pass  # some Genesis versions may not support per-link mass shift
            shifts.append(shift)
        self._current_leg_mass_shifts[envs_idx] = torch.tensor(shifts, device=gs.device)
        self.extras.setdefault("domain_randomization", {})
        self.extras["domain_randomization"]["leg_mass_shifts"] = shifts

    def _randomize_motor_strength(self, envs_idx, t_sample: float):
        """Motor strength multiplicative factor (per-joint)."""
    ...
    def _randomize_delay(self, envs_idx):
        """Randomise per-env action delay (in steps)."""
        n = len(envs_idx)
    ...
    def _apply_push(self):
        ...
        # ---- Countdown timer approach ----
        # When counter fires: sample new force + random duration, start countdown.
        # While countdown > 0: keep applying the stored force.
        # When countdown == 0: apply zero force.

        if self._push_counter % self._push_interval == 0:
            # Sample new push force
            ...
            # Sample random duration (in steps) from configured range
        ...
        # Apply stored force while countdown > 0
        active = (self._push_remaining > 0).unsqueeze(-1).float()  # (num_envs, 1)
    ...
    def _apply_dynamic_payload(self):
        """
        Mid-episode dynamic payload: occasionally changes trunk mass during the episode.
        Simulates someone placing/removing a load on the robot while it's walking.
        Uses curriculum-scaled range.
        """
        if not self._dynamic_payload_enable:
            return

        self._dynamic_payload_counter += 1
        if self._dynamic_payload_counter % self._dynamic_payload_interval != 0:
            return

        # Only apply to a random subset of envs (based on probability)
        mask = torch.rand(self.num_envs, device=self.device) < self._dynamic_payload_prob
        if not mask.any():
            return

        # Use current curriculum level for range
        lvl = float(self.curriculum.level) if self.curriculum.enabled else 1.0
        dp_range = _lerp_range(self.dynamic_payload_easy, self.dynamic_payload_hard, lvl)

        new_shift = float(gs_rand_float(dp_range[0], dp_range[1], (1,), self.device).item())
        try:
            self.robot.set_mass_shift([new_shift], [0])
        except Exception:
            return

        # Update tracking for privileged obs (applied globally, but only makes sense
        # for the envs that got the change — Genesis applies mass globally)
        self._current_mass_shift[:] = new_shift
    ...
    def _store_action(self, action):
        """Store current action in circular buffer."""
    ...
    def _get_delayed_action(self):
        """Retrieve per-env delayed action from circular buffer."""
        # Read index = write_idx - 1 - delay_steps (modulo buffer size)
        # write_idx - 1 is the most recently written action
    ...
    def _resample_commands(self, envs_idx):
        ...
        if self._compound_commands:
            # FIX: sample all 3 commands simultaneously (compound motion)
            ...
            # Legacy: one-at-a-time sampling (not recommended)
            ...
            if vx_mask.any():
                self.commands[envs_idx[vx_mask], 0] = gs_rand_float(
                    self._cmd_cur_ranges["lin_vel_x"][0],
                    self._cmd_cur_ranges["lin_vel_x"][1],
                    (vx_mask.sum(),), gs.device,
                )
            if vy_mask.any():
                self.commands[envs_idx[vy_mask], 1] = gs_rand_float(
                    self._cmd_cur_ranges["lin_vel_y"][0],
                    self._cmd_cur_ranges["lin_vel_y"][1],
                    (vy_mask.sum(),), gs.device,
                )
            if yaw_mask.any():
                self.commands[envs_idx[yaw_mask], 2] = gs_rand_float(
                    self._cmd_cur_ranges["ang_vel"][0],
                    self._cmd_cur_ranges["ang_vel"][1],
                    (yaw_mask.sum(),), gs.device,
                )

        # Standing envs override
    ...
    # ==========================================================
    # PLS: compute per-env, per-joint Kp/Kd from policy stiffness actions
    # ==========================================================

    def _compute_pls_kp_kd(self, stiffness_actions):
        """
        Map 4 per-leg stiffness actions → 12 per-joint Kp, then Kd = 0.2*sqrt(Kp).
        Applies DR multiplicative factors and motor strength on top.

        stiffness_actions: (num_envs, 4) — raw action output
        """
        # Map action → Kp: kp = default + action * scale, clamped to [kp_min, kp_max]
        ...
        # Expand from 4 legs → 12 joints (each leg has 3 joints)
        # leg_joint_map: (4, 3), kp_per_leg: (num_envs, 4)
        kp_per_joint = kp_per_leg[:, :, None].expand(-1, 4, 3).reshape(-1, 12)

        # Kd = 0.2 * sqrt(Kp) — paper Eq. 6
        kd_per_joint = 0.2 * torch.sqrt(kp_per_joint)

        # Apply DR multiplicative factors
    ...
    def step(self, actions):
        ...
        self._target_dof_pos[:] = target_dof_pos  # store for joint_tracking reward

        if self._use_manual_pd:
            # ---- Manual PD torque computation ----
            # Allows per-env, per-joint Kp/Kd (required for PLS + per-joint DR)
            ...
            # Apply torque via force control
            # NOTE: if your Genesis version uses a different API name, adjust here.
            # Common alternatives: set_dofs_force, set_dofs_actuation_force
            self.robot.control_dofs_force(torque, self.motors_dof_idx)
        else:
            # ---- Genesis built-in PD (original approach) ----
        ...
        # ---- Termination (loosened for recovery training) ----
        ...
        # ---- Actor observations ----
        # [ang_vel, projected_gravity + gravity_offset, commands, dof_pos, dof_vel, actions]
        self.obs_buf = torch.cat(
            [
                self.base_ang_vel * self.obs_scales["ang_vel"],
                self.projected_gravity + self._gravity_offset,    # gravity offset DR
                self.commands * self.commands_scale,
                (self.dof_pos - self.default_dof_pos) * self.obs_scales["dof_pos"],
                self.dof_vel * self.obs_scales["dof_vel"],
                self.actions,                                     # full actions (pos + stiffness if PLS)
            ],
            axis=-1,
        )
    ...
    def _build_privileged_obs(self):
        """
        Build privileged observation for the critic.
        Contains everything the actor sees PLUS hidden DR parameters
        and ground-truth values not available on real hardware.

        Layout: [actor_obs, base_lin_vel(3), friction(1),
                 kp_factors(12), kd_factors(12), motor_strength(12),
                 mass_shift(1), com_shift(3), leg_mass_shifts(4),
                 gravity_offset(3), push_force(3), delay_steps(1)]
        """
        self.privileged_obs_buf[:, :self.num_obs] = self.obs_buf

        idx = self.num_obs
        # Base linear velocity (ground truth — not available on real robot)
        ...
        idx += 1
        # Kp/Kd multiplicative factors (per-joint)
    ...
    def reset_idx(self, envs_idx):
        ...
        self._current_leg_mass_shifts[envs_idx] = 0.0
    ...
    def _reward_orientation_penalty(self):
        """Penalize projected gravity xy components (body tilt)."""
        return torch.sum(torch.square(self.projected_gravity[:, :2]), dim=1)

    # ---- NEW: angular velocity xy penalty (damps roll/pitch oscillation) ----
    def _reward_ang_vel_xy(self):
        """Penalize angular velocity in roll and pitch."""
        return torch.sum(torch.square(self.base_ang_vel[:, :2]), dim=1)

    def _reward_stand_still(self):
        """Penalise joint deviation from default when all commands are near zero."""
        cmd_norm = torch.norm(self.commands, dim=1)  # includes yaw
        still_mask = (cmd_norm < 0.1).float()
        return torch.sum(torch.abs(self.dof_pos - self.default_dof_pos), dim=1) * still_mask

    # ---- NEW: penalise base velocity when standing ----
    def _reward_stand_still_vel(self):
        """
        Penalise base xy linear velocity + yaw angular velocity when
        velocity commands are near zero. The robot should not drift or rotate.
        """
        cmd_norm = torch.norm(self.commands, dim=1)  # includes yaw
        still_mask = (cmd_norm < 0.1).float()
    ...
    # ---- NEW: penalise feet lifting when standing ----
    def _reward_feet_stance(self):
        """
        Penalise feet air time when robot should be standing still.
        All 4 feet should remain planted on the ground.
        Only active when commands ≈ 0.
        """
        cmd_norm = torch.norm(self.commands, dim=1)
        still_mask = (cmd_norm < 0.1).float()
        # Count feet in air (not in contact)
        feet_in_air = (~self._foot_contact).float()  # (num_envs, n_feet)
        # Penalise total air time of all feet + number of feet in air
        air_penalty = torch.sum(self._feet_air_time, dim=1) + torch.sum(feet_in_air, dim=1)
        return air_penalty * still_mask

    # ---- feet_air_time (official-style gait reward) ----
    def _reward_feet_air_time(self):
        """
        Reward proper swing/stance timing. Target from reward_cfg["feet_air_time_target"].
        Only active when robot should be moving.
        """
    ...
    # ---- NEW: foot slip (penalise foot xy velocity during ground contact) ----
    def _reward_foot_slip(self):
        """
        Penalize foot sliding while in contact with ground.
        Uses link velocity API if available, otherwise approximates with base velocity.
        """
        contact = self._foot_contact.float()  # (num_envs, n_feet)

        if self._has_link_vel:
            link_vel = self.robot.get_links_vel()  # (num_envs, num_links, 3)
            slip = torch.zeros(self.num_envs, device=self.device, dtype=gs.tc_float)
            for i, link_idx in enumerate(self._foot_link_indices):
                foot_vel_xy = link_vel[:, link_idx, :2]  # (num_envs, 2)
                slip += contact[:, i] * torch.sum(torch.square(foot_vel_xy), dim=1)
            return slip
        else:
            # Fallback: penalise base xy velocity when feet are in contact
            # This is a rough approximation but still useful
    ...
    # ---- NEW: foot clearance (encourage proper foot lifting during swing) ----
    def _reward_foot_clearance(self):
        """
        Penalize feet that don't lift enough during swing phase.
        Uses link position API if available.
        """
        if not self._has_link_pos:
            return torch.zeros(self.num_envs, device=self.device, dtype=gs.tc_float)

        link_pos = self.robot.get_links_pos()  # (num_envs, num_links, 3)
        target_height = self.reward_cfg.get("feet_height_target", 0.075)
        swing = (~self._foot_contact).float()  # (num_envs, n_feet)
        ...
        if self._has_link_vel:
            ...
            for i, link_idx in enumerate(self._foot_link_indices):
                ...
                # Only penalise when foot is moving horizontally (actual swing, not standing)
    ...
    # ---- NEW: joint tracking (PD target vs achieved — actuator feasibility) ----
    def _reward_joint_tracking(self):
        """
        Penalize discrepancy between commanded position and achieved position.
        Encourages policy to command feasible targets that the PD controller can reach.
        Critical for sim-to-real: prevents exploiting sim-only tracking capability.
        """
        return torch.sum(torch.square(self._target_dof_pos - self.dof_pos), dim=1)

    # ---- energy (penalize power consumption) ----
    def _reward_energy(self):
        """Penalize mechanical power: sum(|torque * joint_velocity|)."""
