...
def build_stair_terrain(terrain_cfg: dict) -> tuple:
    ...
    flight_cells = (
        stair_section_cells + flat_top_cells + stair_section_cells + flat_gap_cells
    )
    ...
    print(
        f"  Row length    : {total_x_m:.1f}m  ({stair_pct:.0f}% stairs / {100 - stair_pct:.0f}% flat)"
    )
    print(f"  Terrain origin: {terrain_origin}")
    for i, (cx, cy, cz) in enumerate(adjusted_centers):
        print(
            f"  Row {i} (step={step_heights_m[i] * 100:.1f}cm): spawn=({cx:.2f}, {cy:.2f}, {cz:.2f})"
        )
    print()
...
class CurriculumManager:
    ...
    def update(
        self, timeout_rate: float, tracking_per_sec: float, fall_rate: float
    ) -> bool:
        if not self.enabled:
            return False
    ...
    def state_dict(self):
        return {
            ...
            "timeout_rate_ema": None
            if self.timeout_rate_ema is None
            else float(self.timeout_rate_ema),
            "tracking_ema": None
            if self.tracking_ema is None
            else float(self.tracking_ema),
            "fall_rate_ema": None
            if self.fall_rate_ema is None
            else float(self.fall_rate_ema),
        }
...
class Go2Env:
    def __init__(
        ...
        self._use_terrain = self._terrain_cfg is not None and self._terrain_cfg.get(
            "enabled", False
        )
        ...
        self._env_terrain_row = torch.zeros(
            num_envs, device=gs.device, dtype=torch.long
        )
        ...
        if self._use_terrain:
            ...
            print(
                f"[Go2Env] Terrain loaded: {self._hf.shape} cells, "
        ...
        self.robot.set_dofs_kp(
            [self.env_cfg["kp"]] * self.num_pos_actions, self.motors_dof_idx
        )
        self.robot.set_dofs_kv(
            [self.env_cfg["kd"]] * self.num_pos_actions, self.motors_dof_idx
        )
        ...
        if self._has_leg_mass_dr:
            ...
            if len(self._hip_link_indices) == 4:
                print(
                    f"[Go2Env] Per-leg mass DR ENABLED: hip_link_indices={self._hip_link_indices}"
                )
            else:
        ...
        self.obs_noise_level_max = (
            float(self.env_cfg.get("obs_noise_level", 0.0))
            if self.obs_noise_components
            else 0.0
        )
        self.action_noise_std_max = (
            float(self.env_cfg.get("action_noise_std", 0.0))
        ...
        if self.push_force_range_hard is not None:
            self._all_envs_idx = torch.arange(
            ...
            self._push_stored_force = torch.zeros(
                (self.num_envs, 3), device=gs.device, dtype=gs.tc_float
            )
            self._push_remaining = torch.zeros(
                (self.num_envs,), device=gs.device, dtype=torch.long
            )
        ...
        self._standing_env_indices = (
            torch.arange(n_standing, device=gs.device) if n_standing > 0 else None
        )
        ...
        # Per-env DR buffers
        # ----------------------------------------------------------
        self._kp_factors = torch.ones(
            (self.num_envs, self.num_pos_actions), device=gs.device, dtype=gs.tc_float
        )
        self._kd_factors = torch.ones(
            (self.num_envs, self.num_pos_actions), device=gs.device, dtype=gs.tc_float
        )
        self._motor_strength = torch.ones(
            (self.num_envs, self.num_pos_actions), device=gs.device, dtype=gs.tc_float
        )
        self._gravity_offset = torch.zeros(
            (self.num_envs, 3), device=gs.device, dtype=gs.tc_float
        )
        self._current_push_force = torch.zeros(
            (self.num_envs, 3), device=gs.device, dtype=gs.tc_float
        )
        self._current_friction = torch.ones(
            (self.num_envs,), device=gs.device, dtype=gs.tc_float
        )
        self._current_mass_shift = torch.zeros(
            (self.num_envs,), device=gs.device, dtype=gs.tc_float
        )
        self._current_com_shift = torch.zeros(
            (self.num_envs, 3), device=gs.device, dtype=gs.tc_float
        )
        self._current_leg_mass_shifts = torch.zeros(
            (self.num_envs, 4), device=gs.device, dtype=gs.tc_float
        )

        self._effective_kp = torch.full(
            (self.num_envs, self.num_pos_actions),
            float(self.env_cfg["kp"]),
            device=gs.device,
            dtype=gs.tc_float,
        )
        self._effective_kd = torch.full(
            (self.num_envs, self.num_pos_actions),
            float(self.env_cfg["kd"]),
            device=gs.device,
            dtype=gs.tc_float,
        )
        ...
        self.privileged_obs_buf = torch.zeros(
            (
                self.num_envs,
                self.num_privileged_obs if self.num_privileged_obs else self.num_obs,
            ),
            device=gs.device,
            dtype=gs.tc_float,
        )
        ...
        self.curr_update_every_episodes = int(
            self.curr_cfg.get("update_every_episodes", 2048)
        )
        ...
        self.gravity_offset_hard = self.env_cfg.get(
            "gravity_offset_range", self.gravity_offset_easy
        )

        self.motor_strength_easy = self.curr_cfg.get(
            "motor_strength_easy", [0.97, 1.03]
        )
        self.motor_strength_hard = self.env_cfg.get(
            "motor_strength_range", self.motor_strength_easy
        )

        self.leg_mass_easy = self.curr_cfg.get("leg_mass_shift_easy", [-0.1, 0.1])
        self.leg_mass_hard = self.env_cfg.get(
            "leg_mass_shift_range", self.leg_mass_easy
        )

        self.delay_easy_max = int(self.curr_cfg.get("delay_easy_max_steps", 1))

        self.push_start = float(self.curr_cfg.get("push_start", 0.30))
        self.push_interval_s_easy = float(
            self.curr_cfg.get("push_interval_easy_s", 6.0)
        )

        self._global_dr_update_interval = int(
            self.curr_cfg.get("global_dr_update_interval", 200)
        )
        ...
        self._push_interval = (
            int(self.push_interval_s_easy / self.dt)
            if self.push_force_range_hard is not None
            else 1
        )
        ...
        if self._use_manual_pd:
            ...
            print("[Go2Env] Manual PD torque mode ENABLED")
    ...
    def _assign_terrain_rows(self, envs_idx, curriculum_level: float):
        """Assign each resetting env to a terrain difficulty row based on curriculum level.

        Strategy:
        - 70% of envs go to flat (row 0) or easy rows
        - 30% of envs go to stair rows matching curriculum difficulty
        - As curriculum_level increases, harder rows become available
        """
        ...
        # Max row accessible at current curriculum level
        # level 0.0 → only row 0 (flat/easiest stairs)
        # level 1.0 → all rows accessible
        max_row = int(curriculum_level * (self._num_terrain_rows - 1))
        max_row = max(0, min(max_row, self._num_terrain_rows - 1))

        # 70% of envs on easy rows (0 to max_row//2), 30% on harder rows
        flat_pct = 0.70
        n_flat = int(n * flat_pct)
        n_stairs = n - n_flat

        # Easy rows: 0 to max(0, max_row//2)
        easy_max = max(0, max_row // 2)
        flat_rows = gs_rand_int(0, easy_max, (n_flat,), gs.device)

        # Hard rows: easy_max+1 to max_row (or easy_max if max_row is small)
        hard_min = min(easy_max + 1, max_row)
        stairs_rows = gs_rand_int(hard_min, max_row, (n_stairs,), gs.device)

        assigned_rows = torch.cat([flat_rows, stairs_rows], dim=0)

        # Shuffle so flat/stair envs are mixed
        perm = torch.randperm(n, device=gs.device)
        assigned_rows = assigned_rows[perm]

        self._env_terrain_row[envs_idx] = assigned_rows
    ...
    def _init_foot_contact_tracking(self):
        ...
        for fname in foot_names:
            if fname not in link_name_map:
                raise RuntimeError(
                    f"Foot link '{fname}' NOT FOUND. Available: {sorted(link_name_map.keys())}"
                )
        ...
        for fname, lidx in zip(foot_names, self._foot_link_indices):
            if lidx >= n_links_in_cf:
                raise RuntimeError(
                    f"Foot link '{fname}' idx {lidx} out of range ({n_links_in_cf} links)."
                )
        ...
        self._n_feet = n_feet
        self._foot_contact = torch.zeros(
            self.num_envs, n_feet, device=self.device, dtype=torch.bool
        )
        self._last_foot_contact = torch.zeros_like(self._foot_contact)
        self._feet_air_time = torch.zeros(
            self.num_envs, n_feet, device=self.device, dtype=gs.tc_float
        )
        self._foot_contact_threshold = float(
            self.env_cfg.get("foot_contact_threshold", 1.0)
        )

        print("[Go2Env] Foot contact tracking INITIALIZED:")
    ...
    def _update_foot_contacts(self):
        ...
        for i, link_idx in enumerate(self._foot_link_indices):
            ...
            self._foot_contact[:, i] = (
                torch.abs(foot_force_z) > self._foot_contact_threshold
            )
    ...
    def _apply_curriculum_level(self, force: bool = False):
        ...
        if self.push_force_range_hard is None:
            ...
            if lvl < self.push_start:
                ...
                s = _clamp01(s)
                low_h, high_h = (
                    float(self.push_force_range_hard[0]),
                    float(self.push_force_range_hard[1]),
                )
                self._push_force_range_cur = [low_h * s, high_h * s]
                interval_s = _lerp(
                    self.push_interval_s_easy, self.push_interval_s_hard, s
                )
                self._push_interval = max(1, int(interval_s / self.dt))
                self._push_enable_cur = True

        self._delay_max_cur = int(
            round(
                _lerp(
        ...
        if self._cmd_curriculum:
            ...
            self._cmd_cur_ranges = {
                k: list(v) for k, v in self._cmd_full_ranges.items()
            }
        ...
        self.extras["curriculum"].update(
            {
                ...
                "terrain_max_row": int(lvl * (self._num_terrain_rows - 1))
                if self._use_terrain
                else 0,
            }
        )
    ...
    def _maybe_update_curriculum_on_reset(self, envs_idx):
        ...
        ep_steps = (
            self.episode_length_buf[envs_idx].to(dtype=gs.tc_float).clamp_min(1.0)
        )
        ...
        if self._curr_ep_total >= self.curr_update_every_episodes:
            ...
            tracking_avg = float(
                self._curr_tracking_sum / max(1, self._curr_tracking_n)
            )
    ...
    def _randomize_kp_kd(self, envs_idx, t_sample: float):
        ...
        if self.pls_enable:
            if "kp_factor_range" in self.env_cfg:
                kpf_range = _lerp_range(
                    self.kp_factor_easy, self.kp_factor_hard, t_sample
                )
                self._kp_factors[envs_idx] = gs_rand_float(
                    kpf_range[0],
                    kpf_range[1],
                    (n, self.num_pos_actions),
                    gs.device,
                )
            if "kd_factor_range" in self.env_cfg:
                kdf_range = _lerp_range(
                    self.kd_factor_easy, self.kd_factor_hard, t_sample
                )
            ...
            if "kp_range" in self.env_cfg:
                ...
                if not self._use_manual_pd:
                    ...
                    self.robot.set_dofs_kp(
                        [mean_kp] * self.num_pos_actions, self.motors_dof_idx
                    )
                    self.robot.set_dofs_kv(
                        [mean_kd] * self.num_pos_actions, self.motors_dof_idx
                    )
    ...
    def _randomize_mass(self, envs_idx, t_sample: float):
        if "mass_shift_range" in self.env_cfg:
            ...
            shift = float(
                gs_rand_float(ms_range[0], ms_range[1], (1,), gs.device).item()
            )
    ...
    def _randomize_gravity_offset(self, envs_idx, t_sample: float):
        if "gravity_offset_range" not in self.env_cfg:
            return
        n = len(envs_idx)
        go_range = _lerp_range(
            self.gravity_offset_easy, self.gravity_offset_hard, t_sample
        )
    ...
    def _randomize_leg_mass(self, envs_idx, t_sample: float):
        ...
        for i, link_idx in enumerate(self._hip_link_indices):
            shift = float(
                gs_rand_float(lm_range[0], lm_range[1], (1,), gs.device).item()
            )
            try:
    ...
    def _randomize_motor_strength(self, envs_idx, t_sample: float):
        if "motor_strength_range" not in self.env_cfg:
            return
        n = len(envs_idx)
        ms_range = _lerp_range(
            self.motor_strength_easy, self.motor_strength_hard, t_sample
        )
    ...
    def _randomize_delay(self, envs_idx):
        n = len(envs_idx)
        max_d = max(
            self._min_delay_steps, min(self._delay_max_cur, self._max_delay_steps)
        )
    ...
    def _apply_push(self):
        ...
        if self._push_counter % self._push_interval == 0:
            ...
            self._push_stored_force[:, 0] = gs_rand_float(
                low, high, (self.num_envs,), self.device
            )
            self._push_stored_force[:, 1] = gs_rand_float(
                low, high, (self.num_envs,), self.device
            )
            ...
            self._push_remaining[:] = gs_rand_int(
                dur_lo, dur_hi, (self.num_envs,), self.device
            )
    ...
    def _store_action(self, action):
        ...
        self._action_history_write_idx = (
            self._action_history_write_idx + 1
        ) % self._delay_buf_size

    def _get_delayed_action(self):
        read_idx = (
            self._action_history_write_idx - 1 - self._delay_steps
        ) % self._delay_buf_size
    ...
    def _resample_commands(self, envs_idx):
        ...
        if self._compound_commands:
            ...
            for i, (key, dim) in enumerate(
                zip(["lin_vel_x", "lin_vel_y", "ang_vel"], [0, 1, 2])
            ):
                mask = choice == i
                if mask.any():
        ...
        if (
            self._standing_env_indices is not None
            and len(self._standing_env_indices) > 0
        ):
    ...
    def step(self, actions):
        ...
        target_dof_pos = (
            pos_actions * self.env_cfg["action_scale"] + self.default_dof_pos
        )

        if self._action_noise_std_cur > 0.0:
            target_dof_pos = (
                target_dof_pos
                + torch.randn_like(target_dof_pos) * self._action_noise_std_cur
            )
        ...
        self.reset_buf |= (
            ...
            > self.env_cfg["termination_if_z_vel_greater_than"]
        )
        self.reset_buf |= (
            torch.abs(self.base_lin_vel[:, 1])
            > self.env_cfg["termination_if_y_vel_greater_than"]
        )
    ...
    def _build_privileged_obs(self):
        ...
        idx = self.num_obs
        self.privileged_obs_buf[:, idx : idx + 3] = (
        ...
        if self._max_delay_steps > 0:
            self.privileged_obs_buf[:, idx] = self._delay_steps.float() / float(
                self._max_delay_steps
            )
        idx += 1
        # Terrain difficulty for critic (normalised 0-1)
        # Guard: only write if the buffer was sized to include this dim.
        # Old configs (pre-terrain) have num_privileged_obs that doesn't
        # include terrain_row, so idx would be out of bounds.
        priv_dim = self.privileged_obs_buf.shape[1]
        if self._use_terrain and idx < priv_dim:
            self.privileged_obs_buf[:, idx] = self._env_terrain_row.float() / max(
                1, self._num_terrain_rows - 1
            )
        if idx < priv_dim:
            idx += 1
    ...
    def _reward_feet_stance(self):
        ...
        air_penalty = torch.sum(self._feet_air_time, dim=1) + torch.sum(
            feet_in_air, dim=1
        )
