...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        #
        # Phase 1: terrain_level < gate → DR stays at easy level
        #          (robot focuses purely on learning stairs)
        # Phase 2: terrain_level >= gate → DR ramps from easy → hard
        #          (robot hardens stair policy against disturbances)
        #
        # None = old behaviour (DR tracks curriculum 1:1)
        # ----------------------------------------------------------
    ...
    def _assign_terrain_rows(self, envs_idx, curriculum_level: float):
        ...
        Distribution:
          40% on frontier (max accessible row) — where learning happens
          30% on near-frontier (max_row-1, max_row-2) — consolidation
          30% on easy rows (0 to max_row-3) — flat ground retention
        """
        ...
        # 30% easy (row 0 to max_row-3, for flat/easy retention)
    ...
    def _get_dr_level(self, terrain_level: float) -> float:
        """Compute DR level from terrain curriculum level (two-phase schedule).

        Phase 1 (terrain_level < gate): return phase1_level (fixed, easy)
        Phase 2 (terrain_level >= gate): ramp from phase1_level → 1.0
        """
        if not self._dr_enabled:
            return terrain_level  # Old behaviour: DR = terrain level

        if terrain_level < self._dr_terrain_gate:
            return self._dr_phase1_level
        else:
            # How far past the gate (0.0 = just entered, 1.0 = terrain maxed)
    ...
    def _apply_curriculum_level(self, force: bool = False):
        ...
        # DR level: two-phase schedule based on terrain progress
    ...
    def reset_idx(self, envs_idx):
        ...
        # Terrain level controls stair difficulty
        curriculum_level = self.curriculum.level if self.curriculum.enabled else 1.0

        # DR level: two-phase schedule (easy during stair learning, ramps after)
        t_sample = self._get_dr_level(curriculum_level)

        # Assign terrain rows based on terrain curriculum (NOT dr level)
        ...
        # DR applied per episode (uses phase-appropriate DR level)
    ...
    def _reward_orientation_roll_only(self):
        """Penalize ROLL only (sideways lean), NOT pitch (forward/backward tilt).

        On stairs the robot naturally pitches forward (climbing) or backward
        (descending). Penalising that pitch fights the physics and wastes
        policy capacity. Roll (sideways lean) is always bad — penalise it.

        projected_gravity[:, 0] = x-component → pitch-related (ALLOWED)
        projected_gravity[:, 1] = y-component → roll-related (PENALISED)
        """
