...
class CurriculumManager:
    """
    Metric-gated curriculum for DR / commands (NOT terrain).
    Terrain difficulty is always fully available — no gating.
    """
...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        # ----------------------------------------------------------
        # Terrain Manager — build BEFORE scene
        # ----------------------------------------------------------
        terrain_cfg = env_cfg.get("terrain", {}) or {}
        self.terrain = TerrainManager(terrain_cfg, num_envs, device=gs.device)
        ...
        # Post-build terrain setup (cache height field)
        if self.terrain.enabled:
        ...
        # ----------------------------------------------------------
        # Terrain-aware buffers
        # ----------------------------------------------------------
        self._ground_height = torch.zeros(self.num_envs, device=gs.device, dtype=gs.tc_float)
        ...
        # ----------------------------------------------------------
        # Curriculum (DR + commands only, NOT terrain)
        # ----------------------------------------------------------
    ...
    def step(self, actions):
        ...
        # Update ground height under robot (terrain-aware rewards)
        if self.terrain.enabled:
        ...
        # BOUNDARY RESET: terminate if robot wandered off its assigned tile
        if self.terrain.enabled:
    ...
    # ==========================================================
    # Privileged observations (+ terrain info)
    # ==========================================================
    ...
    def _build_privileged_obs(self):
        ...
        # Terrain: ground height under robot (lets critic understand slope context)
        self.privileged_obs_buf[:, idx] = self._ground_height
        idx += 1
    ...
    def reset_idx(self, envs_idx):
        ...
        # ----------------------------------------------------------
        # Reset base — TERRAIN-AWARE SPAWNING (random tile, no curriculum)
        # ----------------------------------------------------------
        self.base_quat[envs_idx] = self.base_init_quat.reshape(1, -1)

        if self.terrain.enabled:
            base_z = float(self.base_init_pos[2])
            spawn_pos = self.terrain.sample_spawn(envs_idx, base_z)
        ...
        # Log terrain info
        if self.terrain.enabled:
            rows = self.terrain.get_env_tile_rows()
            self.extras["episode"]["terrain_row_mean"] = float(rows[envs_idx].float().mean().item())
    ...
    def _reward_base_height(self):
        """TERRAIN-AWARE: height above local ground, not world z=0."""
    ...
    def _reward_foot_clearance(self):
        ...
        if self._has_link_vel:
            ...
            for i, link_idx in enumerate(self._foot_link_indices):
                ...
                foot_height_above_ground = foot_z - self._ground_height
                foot_vel_xy_norm = torch.norm(link_vel[:, link_idx, :2], dim=1)
                height_error = torch.square(target_height - foot_height_above_ground)
    ...
    def _reward_stumble(self):
        """Penalize non-foot link contacts (knees/thighs hitting terrain)."""
        if not self._has_foot_contact_rewards:
            return torch.zeros(self.num_envs, device=self.device, dtype=gs.tc_float)

        cf = self.robot.get_links_net_contact_force()
        all_contact_mag = torch.norm(cf, dim=-1)  # (num_envs, num_links)
        foot_mask = torch.zeros(all_contact_mag.shape[1], device=self.device, dtype=torch.bool)
        for lidx in self._foot_link_indices:
            foot_mask[lidx] = True

        non_foot_contact = all_contact_mag[:, ~foot_mask]
        threshold = float(self.reward_cfg.get("stumble_threshold", 5.0))
        penalty = torch.sum((non_foot_contact - threshold).clamp(min=0.0), dim=1)
        return penalty
