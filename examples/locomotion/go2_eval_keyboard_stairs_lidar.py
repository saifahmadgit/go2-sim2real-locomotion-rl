...
eval_sim_lidar.py
=================
Interactive evaluation of a LIDAR-aware Go2 stair-climbing policy in Genesis sim.

Adapted for the minimal 5×3 terrain scan grid.  Key details:
  - Actor observation is 64 dims:  49 proprioceptive + 15 LIDAR terrain scan
  - Grid: x=[0.0, 0.15, 0.30, 0.50, 0.80], y=[-0.15, 0.0, 0.15]
  - Model compatibility loader handles dimension mismatches across generations
  - Debug prints show LIDAR scan statistics

Controls are unchanged (P/M/J/K/U/O for movement, 0-9/+/- for terrain).
"""
...
OBS_NOISE_STD = {
    ...
    "dof_vel": 1.5,
    "actions": 0.0,
    # LIDAR scan noise is handled internally by the env's DR pipeline.
    # At eval time we can optionally add a small amount here too.
    "lidar_scan": 0.0,
}
...
def build_obs_noise_vec(obs_dim, noise_cfg, level, num_lidar_scan=0):
    """Build observation noise vector, now aware of LIDAR scan dims.

    Observation layout (LIDAR-aware):
      [0 :  3]  ang_vel
      [3 :  6]  gravity
      [6 :  9]  commands
      [9 : 21]  dof_pos         (12)
      [21: 33]  dof_vel         (12)
      [33: 33+A] actions        (A = num_actions, typically 16)
      [33+A : 33+A+L] lidar_scan (L = num_lidar_scan, typically 15)
    """
    proprio_size = 3 + 3 + 3 + 12 + 12  # 33
    action_size = obs_dim - proprio_size - num_lidar_scan
    ...
    # LIDAR scan portion — the env already applies its own DR noise,
    # but we can add extra eval-time noise here if desired.
    if num_lidar_scan > 0:
        lidar_std = noise_cfg.get("lidar_scan", 0.0)
        parts.append(torch.full((num_lidar_scan,), lidar_std * level, device=gs.device))
...
def load_model_compat(runner, ckpt_path, env):
    """Load a checkpoint, handling dimension mismatches across model generations.

    Handles multiple generations:
    - Walking models (no terrain_row, no height_scan, no LIDAR)
    - Stair v1 models (terrain_row but no height_scan, no LIDAR)
    - Stair v2 models (terrain_row + critic height_scan, no actor LIDAR)
    - Stair v4 LIDAR models: 11×5=55 actor + 11×7=77 critic (old)
    - Stair v4 LIDAR models: 5×3=15 actor + 9×5=45 critic (current)
    """
    ckpt = torch.load(ckpt_path, map_location=gs.device, weights_only=False)
    model_state = ckpt.get("model_state_dict", ckpt)

    # ---- Check ACTOR input dim ----
    actor_key = None
    ...
    if actor_key is not None:
        saved_actor_in = model_state[actor_key].shape[1]
        expected_actor_in = env.num_obs
        if saved_actor_in != expected_actor_in:
            actor_mismatch = expected_actor_in - saved_actor_in
            print("\n[COMPAT] Actor input dim mismatch detected:")
            print(f"  Saved model expects : {saved_actor_in}")
            print(f"  Current env provides: {expected_actor_in}")
            print(f"  Difference          : {actor_mismatch}")
            if saved_actor_in == 49:
                print("  (likely: non-LIDAR model, no terrain scan)")
            elif saved_actor_in == 104:
                print("  (likely: old 11×5=55 LIDAR model, current is 5×3=15)")
            elif saved_actor_in == 64:
                print("  (likely: 5×3=15 LIDAR model)")
            print("  ⚠ Cannot load actor — dim mismatch is fatal for inference!")
            print("  ⚠ Train from scratch with matching LIDAR config.\n")
            raise RuntimeError(
                f"Actor input dim mismatch: saved={saved_actor_in} vs "
                f"expected={expected_actor_in}. "
                f"LIDAR-aware policy requires a matching checkpoint."
            )

    # ---- Check CRITIC input dim ----
    critic_key = None
    for k in model_state:
    ...
    if critic_key is not None:
        ...
        if saved_critic_in != expected_critic_in:
            critic_mismatch = expected_critic_in - saved_critic_in
            ...
            print(f"  Difference          : {critic_mismatch}")
            if abs(critic_mismatch) == 77 or abs(critic_mismatch) == 45:
                print("  (likely: model with different critic LIDAR scan size)")
            elif abs(critic_mismatch) == 1:
...
def _partial_load(runner, model_state):
    """Load only matching weights, skip mismatched layers."""
...
def main():
    ...
    # ================================================================
    # Detect training config features
    # ================================================================
    ...
    trained_with_lidar = (
        "lidar" in env_cfg
        and isinstance(env_cfg["lidar"], dict)
        and env_cfg["lidar"].get("enabled", False)
    )
    ...
    # Disable training-time DR and curriculum (but keep LIDAR config!)
    for key in [
        "friction_range",
        "kp_range",
        "kd_range",
    ...
    # --- CRITICAL: Disable env-internal action delay ---
    # The eval script has its own ACTION_DELAY buffer. If we leave the
    # env's internal delay active, delays STACK: eval(1) + env(0-2) = 1-3 steps.
    # Training only had 0-1 steps total. Fix: zero out env delay, let eval handle it.
    env_cfg["min_delay_steps"] = 0
    env_cfg["max_delay_steps"] = 0

    # --- CRITICAL: Disable DR schedule so dr_level doesn't go to 1.0 ---
    # With curriculum disabled, lvl=1.0 → dr_level=1.0 → delay_max_cur ramps to max.
    # Remove dr_schedule so _dr_enabled=False and dr_level stays at terrain_level.
    env_cfg.pop("dr_schedule", None)

    if "curriculum" in env_cfg:
        env_cfg["curriculum"]["enabled"] = False

    # Disable LIDAR noise at eval time (env handles it via DR level = 0)
    # The env's _lidar_noise_level starts at 0.0, so no noise is applied
    # unless curriculum ramps it up (which we disabled above).
    # Keep the lidar config itself intact so the scan is computed.
    if "lidar" in env_cfg and isinstance(env_cfg["lidar"], dict):
        lidar_noise = env_cfg["lidar"].get("noise", {})
        # Zero out all LIDAR noise for clean eval
        lidar_noise["height_noise_std"] = 0.0
        lidar_noise["dropout_prob"] = 0.0
        lidar_noise["full_blackout_prob"] = 0.0
        lidar_noise["latency_prob"] = 0.0
        lidar_noise["vertical_offset_std"] = 0.0
        env_cfg["lidar"]["noise"] = lidar_noise
    ...
    if args.no_terrain:
        ...
        # If terrain is off, LIDAR scan will return zeros (no heightfield)
        if trained_with_lidar:
            print("[WARN] LIDAR-trained model but terrain OFF — scan will be zeros!")
    ...
    # ================================================================
    # Detect LIDAR config from the env
    # ================================================================
    lidar_enabled = env._lidar.enabled if hasattr(env, "_lidar") else False
    num_actor_lidar = env._lidar.num_actor_scan if lidar_enabled else 0
    ...
    # ================================================================
    print("\n" + "=" * 64)
    print("  INFERENCE CONFIG — LIDAR + STAIR TERRAIN + KEYBOARD CONTROL")
    print("=" * 64)
    ...
    if lidar_enabled:
        print(
            f"  Actor LIDAR scan : {num_actor_lidar} points "
            f"({env._lidar._actor_nx}×{env._lidar._actor_ny})"
        )
        print(
            f"  Critic LIDAR scan: {env._lidar.num_critic_scan} points "
            f"({env._lidar._critic_nx}×{env._lidar._critic_ny})"
        )
        print("  LIDAR noise      : OFF (eval mode)")
    ...
    print(f"  Trained w/LIDAR  : {trained_with_lidar}")
    ...
    print_controls()
    print("Running LIDAR evaluation with keyboard control...\n")
    ...
    try:
        with torch.no_grad():
            while True:
                ...
                if OBS_NOISE_ENABLE:
                    if obs_noise_vec is None:
                        obs_noise_vec = build_obs_noise_vec(
                            ...
                            OBS_NOISE_STD,
                            OBS_NOISE_LEVEL,
                            num_lidar_scan=num_actor_lidar,
                        )
                ...
                # ==========================================================
                #  DEBUG PRINTS (with LIDAR scan stats)
                # ==========================================================
                if DEBUG_PRINT_INTERVAL > 0 and step % DEBUG_PRINT_INTERVAL == 0:
                    ...
                    # LIDAR scan stats  ← NEW
                    if lidar_enabled and hasattr(env, "_actor_scan_buf"):
                        scan = env._actor_scan_buf[0]
                        scan_min = scan.min().item()
                        scan_max = scan.max().item()
                        scan_mean = scan.mean().item()
                        nonzero = (scan.abs() > 0.01).sum().item()
                        parts.append(
                            f"lidar=[{scan_min:.2f},{scan_max:.2f}] "
                            f"μ={scan_mean:.2f} nz={nonzero}/{num_actor_lidar}"
                        )
