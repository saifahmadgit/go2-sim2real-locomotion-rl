...
# ==============================================================================
# STAIRWELL terrain builder (vectorized)
# ==============================================================================

def build_stairwell_terrain(terrain_cfg: dict) -> tuple:
    """
    Build a grid of stairwells — inverted square pyramids with stairs on
    all 4 sides, two flights per side with a flat landing between them.

    Profile (center → rim):
      flat_bottom → flight_1 (stairs up) → landing → flight_2 (stairs up) → ground

    Robot spawns at flat_bottom. Walk any direction → climb stairs.
    """
    h_scale = terrain_cfg.get("horizontal_scale", 0.05)
    v_scale = terrain_cfg.get("vertical_scale", 0.005)

    step_depth_m = terrain_cfg.get("step_depth_m", 0.39)
    steps_per_flight = terrain_cfg.get("steps_per_flight", 10)
    num_flights = terrain_cfg.get("num_flights_per_side", 2)
    flat_landing_m = terrain_cfg.get("flat_landing_m", 1.5)
    flat_bottom_m = terrain_cfg.get("flat_bottom_m", 2.0)
    flat_surround_m = terrain_cfg.get("flat_surround_m", 4.0)

    step_height_min = terrain_cfg.get("step_height_min", 0.02)
    step_height_max = terrain_cfg.get("step_height_max", 0.135)
    num_difficulty_levels = terrain_cfg.get("num_difficulty_levels", 9)
    wells_per_row = terrain_cfg.get("wells_per_row", 3)

    # Convert to cells
    step_depth_cells = max(1, int(round(step_depth_m / h_scale)))
    flat_bottom_cells = max(1, int(round(flat_bottom_m / h_scale)))
    flat_landing_cells = max(1, int(round(flat_landing_m / h_scale)))
    flat_surround_cells = max(1, int(round(flat_surround_m / h_scale)))
    flight_cells = steps_per_flight * step_depth_cells
    half_bottom = flat_bottom_cells // 2

    # Radius from center to rim
    # For 2 flights: half_bottom + flight + landing + flight
    well_radius_cells = half_bottom
    for _f in range(num_flights):
        well_radius_cells += flight_cells
        if _f < num_flights - 1:
            well_radius_cells += flat_landing_cells

    well_body_cells = 2 * well_radius_cells + 1
    well_block_cells = well_body_cells + flat_surround_cells

    num_well_rows = math.ceil(num_difficulty_levels / wells_per_row)

    total_x_cells = wells_per_row * well_block_cells + flat_surround_cells
    total_y_cells = num_well_rows * well_block_cells + flat_surround_cells

    # Step heights per difficulty
    step_heights_m = np.linspace(step_height_min, step_height_max, num_difficulty_levels)
    step_heights_units = np.round(step_heights_m / v_scale).astype(np.int32)

    total_steps = steps_per_flight * num_flights
    max_well_depth = int(total_steps * step_heights_units[-1])
    ground_height = max_well_depth

    hf = np.full((total_x_cells, total_y_cells), ground_height, dtype=np.int16)

    # Precompute coordinate grids
    ix_grid, iy_grid = np.meshgrid(
        np.arange(total_x_cells), np.arange(total_y_cells), indexing='ij'
    )

    # Per-difficulty well data
    well_spawns = {}  # level → {"center": (wx, wy, wz), "step_height_m": float}
    well_idx = 0

    for wy in range(num_well_rows):
        for wx in range(wells_per_row):
            if well_idx >= num_difficulty_levels:
                break

            sh_units = int(step_heights_units[well_idx])
            total_depth_units = total_steps * sh_units
            bottom_height = ground_height - total_depth_units

            # Well center
            cx = flat_surround_cells // 2 + wx * well_block_cells + well_radius_cells
            cy = flat_surround_cells // 2 + wy * well_block_cells + well_radius_cells

            # Chebyshev distance
            dist = np.maximum(np.abs(ix_grid - cx), np.abs(iy_grid - cy))

            # Build height map for this well
            h = np.full_like(dist, ground_height, dtype=np.int32)

            # Flat bottom
            mask = dist <= half_bottom
            h[mask] = bottom_height

            # Process flights and landings
            boundary = half_bottom
            current_base_height = bottom_height

            for flight_idx in range(num_flights):
                # Stairs for this flight
                stair_start = boundary
                stair_end = boundary + flight_cells

                mask = (dist > stair_start) & (dist <= stair_end)
                if np.any(mask):
                    step_idx = np.clip(
                        (dist[mask] - stair_start - 1) // step_depth_cells,
                        0, steps_per_flight - 1
                    )
                    h[mask] = current_base_height + (step_idx + 1) * sh_units

                current_base_height += steps_per_flight * sh_units
                boundary += flight_cells

                # Flat landing (except after last flight)
                if flight_idx < num_flights - 1:
                    landing_end = boundary + flat_landing_cells
                    mask = (dist > boundary) & (dist <= landing_end)
                    h[mask] = current_base_height
                    boundary += flat_landing_cells

            # Take min (well cuts into ground)
            hf = np.minimum(hf, h.astype(np.int16))

            # World coords for spawn
            terrain_origin_x = -total_x_cells * h_scale / 2.0
            terrain_origin_y = -total_y_cells * h_scale / 2.0
            world_x = cx * h_scale + terrain_origin_x
            world_y = cy * h_scale + terrain_origin_y
            world_z = bottom_height * v_scale

            well_spawns[well_idx] = {
                "center": (world_x, world_y, world_z),
                "step_height_m": float(step_heights_m[well_idx]),
            }
    ...
    terrain_origin = (-total_x_m / 2.0, -total_y_m / 2.0, 0.0)
    ...
    terrain_info = {
        ...
        "well_spawns": well_spawns,
        "step_heights_m": step_heights_m.tolist(),
        "ground_height_m": ground_height * v_scale,
        "steps_per_flight": steps_per_flight,
        "num_flights_per_side": num_flights,
        "step_depth_m": step_depth_m,
    }

    well_radius_m = well_radius_cells * h_scale
    print(f"\n[Terrain] Built STAIRWELL terrain:")
    print(f"  Shape            : {hf.shape} cells ({total_x_m:.1f}m × {total_y_m:.1f}m)")
    print(f"  Wells            : {num_difficulty_levels} ({wells_per_row}×{num_well_rows})")
    print(f"  Profile/side     : bottom → {steps_per_flight} stairs → "
          f"{'landing → ' * (num_flights - 1)}{steps_per_flight} stairs → ground")
    print(f"  Step depth       : {step_depth_m*100:.0f}cm")
    print(f"  Well radius      : {well_radius_m:.1f}m")
    print(f"  Ground height    : {ground_height * v_scale:.3f}m")
    for lvl, info in well_spawns.items():
        cx, cy, cz = info["center"]
        print(f"  Level {lvl:2d}: step={info['step_height_m']*100:5.1f}cm, "
              f"spawn=({cx:+.1f}, {cy:+.1f}, {cz:.3f})")
    print()
...
class CurriculumManager:
    ...
    def update(self, timeout_rate, tracking_per_sec, fall_rate):
        if not self.enabled:
            return False
...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        if torque_lim_cfg is not None:
            self._torque_limits = torch.tensor(torque_lim_cfg, device=gs.device, dtype=gs.tc_float)
        else:
            self._torque_limits = torch.full((self.num_pos_actions,), 23.7, device=gs.device, dtype=gs.tc_float)
        ...
        if self._use_terrain:
            self._hf, self._terrain_info = build_stairwell_terrain(self._terrain_cfg)
            self._num_difficulty_levels = self._terrain_info["num_difficulty_levels"]
            self._well_spawns = self._terrain_info["well_spawns"]
            ...
            print(f"[Go2Env] Height scan: {self._scan_nx}×{self._scan_ny} = {self._scan_n} points")
        else:
            self._num_difficulty_levels = 1
            self._well_spawns = {0: {"center": (0.0, 0.0, 0.0), "step_height_m": 0.0}}
            self._scan_n = 0

        # Per-env well assignment
        self._env_well_level = torch.zeros(num_envs, device=gs.device, dtype=torch.long)
        ...
        for name in self.reward_scales.keys():
            ...
            if not hasattr(self, fn_name):
                raise AttributeError(f"Reward '{fn_name}' not found. Remove '{name}' from reward_scales.")
        ...
        if self._has_leg_mass_dr:
            ...
            for pattern in [["FR_hip", "FL_hip", "RR_hip", "RL_hip"],
            ...
            if not self._hip_link_indices:
                for jname in ["FR_hip_joint", "FL_hip_joint", "RR_hip_joint", "RL_hip_joint"]:
                    try:
                        joint = self.robot.get_joint(jname)
                        li = getattr(joint, "child_link_idx", getattr(joint, "link_idx", None))
                        if li is not None:
                            self._hip_link_indices.append(int(li))
                    except Exception:
                        pass

            if len(self._hip_link_indices) == 4:
                print(f"[Go2Env] Per-leg mass DR ENABLED")
            else:
        ...
        if self.push_force_range_hard is not None:
            ...
            self._push_duration_steps = [max(1, int(dur_cfg[0] / self.dt)), max(1, int(dur_cfg[1] / self.dt))]
        ...
        self._action_history = torch.zeros(
            (self.num_envs, self._delay_buf_size, self.num_actions), device=gs.device, dtype=gs.tc_float)
    ...
    def _compute_height_scan(self):
        if not self._use_terrain or self._scan_n == 0:
            return torch.zeros(self.num_envs, max(self._scan_n, 1), device=self.device, dtype=gs.tc_float)
        ...
        return terrain_h - self.base_pos[:, 2:3]

    # ==========================================================
    # Well assignment & spawn
    # ==========================================================

    def _assign_wells(self, envs_idx, curriculum_level: float):
        """Frontier-heavy well assignment (same as original terrain rows)."""
        if not self._use_terrain or self._num_difficulty_levels <= 1:
            return
        n = len(envs_idx)
        if n == 0:
            return

        max_level = int(curriculum_level * (self._num_difficulty_levels - 1))
        max_level = max(0, min(max_level, self._num_difficulty_levels - 1))

        levels = torch.zeros(n, device=gs.device, dtype=torch.long)
        n_frontier = int(n * 0.40)
        n_near = int(n * 0.30)

        levels[:n_frontier] = max_level

        if max_level >= 2:
            levels[n_frontier:n_frontier + n_near] = gs_rand_int(
                max(0, max_level - 2), max(0, max_level - 1), (n_near,), gs.device)
        else:
            levels[n_frontier:n_frontier + n_near] = max_level

        n_easy = n - n_frontier - n_near
        easy_max = max(0, max_level - 3) if max_level >= 3 else 0
        levels[n_frontier + n_near:] = gs_rand_int(0, easy_max, (n_easy,), gs.device)

        perm = torch.randperm(n, device=gs.device)
        self._env_well_level[envs_idx] = levels[perm]

    def _get_spawn_pos(self, envs_idx):
        """Spawn at bottom of assigned well."""
        n = len(envs_idx)
        if not self._use_terrain:
            return self.base_init_pos.unsqueeze(0).expand(n, -1).clone()

        spawn_pos = torch.zeros((n, 3), device=gs.device, dtype=gs.tc_float)
        levels = self._env_well_level[envs_idx]

        for i in range(n):
            lvl = int(levels[i].item())
            cx, cy, cz = self._well_spawns[lvl]["center"]
            # Small random offset within flat bottom (±0.3m)
    ...
    def _init_foot_contact_tracking(self):
        ...
        if not foot_names:
            raise ValueError("env_cfg['foot_names'] required.")
        ...
        for fname in foot_names:
            if fname not in link_name_map:
                raise RuntimeError(f"Foot '{fname}' NOT FOUND. Available: {sorted(link_name_map.keys())}")
            self._foot_link_indices.append(link_name_map[fname])

        cf = self.robot.get_links_net_contact_force()
        if cf is None:
            raise RuntimeError("Contact force API not available.")
        ...
        print(f"[Go2Env] Foot contact tracking: {foot_names}")
    ...
    def _update_foot_contacts(self):
        ...
        for i, link_idx in enumerate(self._foot_link_indices):
            self._foot_contact[:, i] = torch.abs(cf[:, link_idx, 2]) > self._foot_contact_threshold
    ...
    def _apply_curriculum_level(self, force=False):
        ...
        if self.push_force_range_hard is None:
            ...
            if dr_lvl < self.push_start:
                ...
                s = _clamp01((dr_lvl - self.push_start) / max(1e-6, 1.0 - self.push_start))
        ...
        self._delay_max_cur = int(round(_lerp(float(self.delay_easy_max), float(self._max_delay_steps), dr_lvl)))
        ...
        self.extras["curriculum"].update({
            ...
            "terrain_max_level": int(lvl * (self._num_difficulty_levels - 1)) if self._use_terrain else 0,
        })
    ...
    def _maybe_update_curriculum_on_reset(self, envs_idx):
        ...
        timeouts = float(self.extras.get("time_outs", torch.zeros(1))[envs_idx].sum().item()) if "time_outs" in self.extras else 0.0
        ...
        if "tracking_lin_vel" in self.episode_sums:
            tracking_int += self.episode_sums["tracking_lin_vel"][envs_idx]
        if "tracking_ang_vel" in self.episode_sums:
            tracking_int += self.episode_sums["tracking_ang_vel"][envs_idx]

        tracking_per_sec = tracking_int / ep_seconds
        self._curr_ep_total += n
        self._curr_timeout_total += timeouts
        self._curr_tracking_sum += float(tracking_per_sec.sum().item())
    ...
    def _randomize_friction(self, envs_idx, t):
        ...
        if self._global_dr_reset_counter < self._global_dr_update_interval:
            return
        self._global_dr_reset_counter = 0
        mu_range = _lerp_range(self.friction_easy, self.friction_hard, t)
    ...
    def _randomize_kp_kd(self, envs_idx, t):
        n = len(envs_idx)
        if self.pls_enable:
            if "kp_factor_range" in self.env_cfg:
                r = _lerp_range(self.kp_factor_easy, self.kp_factor_hard, t)
                self._kp_factors[envs_idx] = gs_rand_float(r[0], r[1], (n, self.num_pos_actions), gs.device)
            if "kd_factor_range" in self.env_cfg:
                r = _lerp_range(self.kd_factor_easy, self.kd_factor_hard, t)
                self._kd_factors[envs_idx] = gs_rand_float(r[0], r[1], (n, self.num_pos_actions), gs.device)
        else:
            if "kp_range" in self.env_cfg:
                kp_r = _lerp_range(self.kp_easy, self.kp_hard, t)
                kp_val = gs_rand_float(kp_r[0], kp_r[1], (n, 1), gs.device).expand(n, self.num_pos_actions)
                kd_r = _lerp_range(self.kd_easy, self.kd_hard, t)
                kd_val = gs_rand_float(kd_r[0], kd_r[1], (n, 1), gs.device).expand(n, self.num_pos_actions)
                ...
                self._effective_kp[envs_idx] = kp_val * self._kp_factors[envs_idx]
                self._effective_kd[envs_idx] = kd_val * self._kd_factors[envs_idx]

    def _randomize_mass(self, envs_idx, t):
        if "mass_shift_range" in self.env_cfg:
            r = _lerp_range(self.mass_easy, self.mass_hard, t)
            shift = float(gs_rand_float(r[0], r[1], (1,), gs.device).item())
            self.robot.set_mass_shift([shift], [0])
            self._current_mass_shift[:] = shift
        if "com_shift_range" in self.env_cfg:
            r = _lerp_range(self.com_easy, self.com_hard, t)
            d = [float(gs_rand_float(r[0], r[1], (1,), gs.device).item()) for _ in range(3)]
            self.robot.set_COM_shift([d], [0])
            self._current_com_shift[:] = torch.tensor(d, device=gs.device)

    def _randomize_gravity_offset(self, envs_idx, t):
        if "gravity_offset_range" not in self.env_cfg:
            return
        n = len(envs_idx)
        r = _lerp_range(self.gravity_offset_easy, self.gravity_offset_hard, t)
        self._gravity_offset[envs_idx] = gs_rand_float(r[0], r[1], (n, 3), gs.device)

    def _randomize_leg_mass(self, envs_idx, t):
        if not self._has_leg_mass_dr:
            return
        r = _lerp_range(self.leg_mass_easy, self.leg_mass_hard, t)
        shifts = []
        for link_idx in self._hip_link_indices:
            s = float(gs_rand_float(r[0], r[1], (1,), gs.device).item())
            try:
                self.robot.set_mass_shift([s], [link_idx])
            except Exception:
                pass
            shifts.append(s)
        self._current_leg_mass_shifts[:] = torch.tensor(shifts, device=gs.device)

    def _randomize_motor_strength(self, envs_idx, t):
        if "motor_strength_range" not in self.env_cfg:
            return
        n = len(envs_idx)
        r = _lerp_range(self.motor_strength_easy, self.motor_strength_hard, t)
        self._motor_strength[envs_idx] = gs_rand_float(r[0], r[1], (n, self.num_pos_actions), gs.device)
    ...
    def _apply_push(self):
        ...
        if self._push_counter % self._push_interval == 0:
            ...
            self._push_remaining[:] = gs_rand_int(
                self._push_duration_steps[0], self._push_duration_steps[1], (self.num_envs,), self.device)
    ...
    def _get_delayed_action(self):
        ...
        return self._action_history[torch.arange(self.num_envs, device=gs.device), read_idx, :]
    ...
    def _resample_commands(self, envs_idx):
        ...
        if self._compound_commands:
            for i, key in enumerate(["lin_vel_x", "lin_vel_y", "ang_vel"]):
                self.commands[envs_idx, i] = gs_rand_float(
                    self._cmd_cur_ranges[key][0], self._cmd_cur_ranges[key][1], (n,), gs.device)
        else:
            ...
            for i, key in enumerate(["lin_vel_x", "lin_vel_y", "ang_vel"]):
                mask = (choice == i)
                if mask.any():
                    self.commands[envs_idx[mask], i] = gs_rand_float(
                        self._cmd_cur_ranges[key][0], self._cmd_cur_ranges[key][1], (mask.sum(),), gs.device)

        if self._standing_env_indices is not None:
    ...
    def _compute_pls_kp_kd(self, stiffness_actions):
        ...
        return kp_per_joint * self._kp_factors * self._motor_strength, kd_per_joint * self._kd_factors
    ...
    def step(self, actions):
        ...
        if self._use_manual_pd:
            if self.pls_enable:
                self._effective_kp, self._effective_kd = self._compute_pls_kp_kd(
                    delayed_actions[:, self.num_pos_actions:])
    ...
    def _build_privileged_obs(self):
        ...
        idx += 1
        self.privileged_obs_buf[:, idx:idx+12] = self._kp_factors;  idx += 12
        self.privileged_obs_buf[:, idx:idx+12] = self._kd_factors;  idx += 12
        self.privileged_obs_buf[:, idx:idx+12] = self._motor_strength;  idx += 12
        self.privileged_obs_buf[:, idx] = self._current_mass_shift;  idx += 1
        self.privileged_obs_buf[:, idx:idx+3] = self._current_com_shift;  idx += 3
        self.privileged_obs_buf[:, idx:idx+4] = self._current_leg_mass_shifts;  idx += 4
        self.privileged_obs_buf[:, idx:idx+3] = self._gravity_offset;  idx += 3
        self.privileged_obs_buf[:, idx:idx+3] = self._current_push_force;  idx += 3
        if self._max_delay_steps > 0:
            self.privileged_obs_buf[:, idx] = self._delay_steps.float() / float(self._max_delay_steps)
        idx += 1

        # Well difficulty (normalised 0-1)
        if idx < priv_dim:
            if self._use_terrain:
                self.privileged_obs_buf[:, idx] = (
                    self._env_well_level.float() / max(1, self._num_difficulty_levels - 1))
            idx += 1

        # Height scan
        if self._use_terrain and self._scan_n > 0 and (idx + self._scan_n) <= priv_dim:
            self.privileged_obs_buf[:, idx:idx + self._scan_n] = self._compute_height_scan()
            idx += self._scan_n
    ...
    def get_privileged_observations(self):
        return self.privileged_obs_buf if self.num_privileged_obs else None
    ...
    def reset_idx(self, envs_idx):
        ...
        if not self._lock_terrain_rows:
            self._assign_wells(envs_idx, curriculum_level)
        ...
        self.robot.set_dofs_position(
            position=self.dof_pos[envs_idx], dofs_idx_local=self.motors_dof_idx,
            zero_velocity=True, envs_idx=envs_idx)

        # Spawn at well bottom
        spawn_pos = self._get_spawn_pos(envs_idx)
        ...
        if "init_euler_range" in self.env_cfg:
            ...
            yaw = gs_rand_float(low_rad, high_rad, (n,), gs.device)
        ...
        if self._use_terrain:
            self.extras["episode"]["terrain_mean_level"] = self._env_well_level[envs_idx].float().mean().item()
    ...
    def _reward_lin_vel_z(self):
        ...
        dz = float(self.reward_cfg.get("lin_vel_z_deadzone", 0.0))
        if dz > 0.0:
            return torch.square(torch.clamp(torch.abs(z_vel) - dz, min=0.0))
    ...
    def _reward_base_height(self):
        if self._use_terrain:
            ...
            h = self.base_pos[:, 2] - terrain_z
        else:
            h = self.base_pos[:, 2]
        return torch.square(h - self.reward_cfg["base_height_target"])
    ...
    def _reward_stand_still(self):
        still = (torch.norm(self.commands, dim=1) < 0.1).float()
        return torch.sum(torch.abs(self.dof_pos - self.default_dof_pos), dim=1) * still

    def _reward_stand_still_vel(self):
        still = (torch.norm(self.commands, dim=1) < 0.1).float()
        lin = torch.sum(torch.square(self.base_lin_vel[:, :2]), dim=1)
        ang = torch.square(self.base_ang_vel[:, 2])
        return (lin + 0.5 * ang) * still

    def _reward_feet_stance(self):
        still = (torch.norm(self.commands, dim=1) < 0.1).float()
        air = (~self._foot_contact).float()
        return (torch.sum(self._feet_air_time, dim=1) + torch.sum(air, dim=1)) * still
    ...
    def _reward_feet_air_time(self):
        ...
        return rew * (torch.norm(self.commands[:, :2], dim=1) > 0.1).float()
    ...
    def _reward_foot_slip(self):
        ...
        if self._has_link_vel:
            ...
            for i, li in enumerate(self._foot_link_indices):
                slip += contact[:, i] * torch.sum(torch.square(link_vel[:, li, :2]), dim=1)
            return slip
        any_c = contact.sum(dim=1).clamp(max=1.0)
        return any_c * torch.sum(torch.square(self.base_lin_vel[:, :2]), dim=1)
    ...
    def _reward_foot_clearance(self):
        ...
        target = self.reward_cfg.get("feet_height_target", 0.075)
        swing = (~self._foot_contact).float()
        penalty = torch.zeros(self.num_envs, device=self.device, dtype=gs.tc_float)
        link_vel = self.robot.get_links_vel() if self._has_link_vel else None

        for i, li in enumerate(self._foot_link_indices):
            fz = link_pos[:, li, 2]
            if self._use_terrain:
                fz = fz - self._get_terrain_height(link_pos[:, li, 0], link_pos[:, li, 1])
            he = torch.square(target - fz)
            if link_vel is not None:
                he = he * torch.norm(link_vel[:, li, :2], dim=1)
            penalty += swing[:, i] * he

        return penalty * (torch.norm(self.commands[:, :2], dim=1) > 0.1).float()
    ...
    def _reward_energy(self):
        ...
        return torch.sum(torch.abs(tau * self.dof_vel), dim=1)
