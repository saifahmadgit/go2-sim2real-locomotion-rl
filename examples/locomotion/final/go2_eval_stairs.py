...
def main():
    ...
    if GRAVITY_OBS_OFFSET_ENABLE:
        gravity_obs_offset = torch.tensor(GRAVITY_OBS_OFFSET, device=gs.device, dtype=gs.tc_float).unsqueeze(0)
    ...
    if not pls_enabled:
        ...
        print(f"  Kp              : policy-controlled [{env_cfg['pls_kp_range'][0]}, {env_cfg['pls_kp_range'][1]}]")
    ...
    try:
        with torch.no_grad():
            while True:
                ...
                if PUSH_ENABLE and step % push_interval_steps == 0 and push_remaining == 0:
                    ...
                    push_remaining = int(rand_int(push_dur_steps_range[0], push_dur_steps_range[1], (1,)).item())
                ...
                if OBS_NOISE_ENABLE:
                    if obs_noise_vec is None:
                        obs_noise_vec = build_obs_noise_vec(obs.shape[-1], OBS_NOISE_STD, OBS_NOISE_LEVEL)
                ...
                if ACTION_NOISE_ENABLE:
                    ...
                    pos_noise = torch.randn(1, num_pos, device=gs.device) * ACTION_NOISE_STD
                ...
                if DEBUG_PRINT_INTERVAL > 0 and step % DEBUG_PRINT_INTERVAL == 0:
                    ...
                    cmd = target_cmd[0]
                    parts.append(f"cmd=[{cmd[0].item():+.2f},{cmd[1].item():+.2f},{cmd[2].item():+.2f}]")
                    ...
                    if pls_enabled:
                        ...
                        kp_per_leg = torch.clamp(kp_per_leg, env.pls_kp_range[0], env.pls_kp_range[1])
                        parts.append(
